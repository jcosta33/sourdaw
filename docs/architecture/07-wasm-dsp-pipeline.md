...
Publish a Scoring or ProofChamber source change through `pnpm lane:publish` to request a hosted build. The independent **Hosted WASM artifacts** pull-request workflow checks out a credentialless control root at the workflow revision and a separate credentialless source root at the exact PR head. The control helper is run from the control root, but imports the source root's WASM toolkit and runs every source-dependent command there. This permits a workflow revision to qualify an older source head that predates the helper while keeping the source manifest, closure hashes, scripts and toolchain pins authoritative. It selects stale supported packages by the existing crate-closure fingerprint. Changes to the matching generator select its package; changes to shared generation inputs or the hosted workflow/helper select both. Unsupported stale packages fail explicitly. A current head with no generation changes reports no work and uploads nothing.
...
2. From a trusted control checkout, run `node scripts/hostedWasmArtifacts.ts verify-return /absolute/source-root /absolute/artifact.zip /absolute/run.json /absolute/artifact.json OWNER/REPO PR_NUMBER RUN_ID ARTIFACT_ID /absolute/private/verified-output`. The source root's HEAD must still match the built head and its tracked and untracked source must be clean. The final directory must not exist and must be outside the source checkout. This checks the archive digest against API metadata, run/head/attempt identity, ZIP structure and size limits, canonical regular-file members, complete package membership, and receipt hashes before publishing any verified files. It neither executes downloaded or source-checkout scripts nor changes the source lane.

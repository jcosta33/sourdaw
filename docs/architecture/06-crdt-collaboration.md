...
2. **WebSocket relay.** A standalone package at repo-root `server/` (`sourdaw-collab-server`): a small `ws` relay on port 8787 handling session/peer registry, host migration, and the message types `join/leave/action/cursor/sync-request/sync-response/state-update`. It is deliberately *not* in the pnpm workspace (own `package-lock.json`, own tsconfig, built in CI by `scripts/health-gates-server.sh`). Startup requires a 32–128 character base64url `COLLAB_AUTH_TOKEN`; clients send `sourdaw` and that token as WebSocket subprotocols. The relay binds `127.0.0.1` unless `COLLAB_HOST` is explicitly set and applies configurable connection, session, peer, payload, and rate limits.

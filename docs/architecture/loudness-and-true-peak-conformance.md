...
`src/utils/audioMetering/{measureIntegratedLoudness,createKWeightingFilters}.ts`.
...
it is all-in — _"including any pass-band ripple in the upsampling filter and the 'under-read'
described in ITU-R BS.1770."_
...
is a rate the recommendation's _illustration_ reaches; it is not a requirement.
...
_loudness_ minimum-requirement cases and does not apply to true peak.
...
The same 4× filter is **shared with the limiter's gain computer by deliberate design** (_"the dBTP
the meter shows and the dBTP the limiter enforces come from one filter"_), so raising the factor is a
...
- _Unverified:_ which ITU revision introduced the two-gate conjunction. It is present in -4/-5 and

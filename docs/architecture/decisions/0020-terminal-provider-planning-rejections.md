...
The provider-neutral planner already exposes rejected outcomes, but the prompt bar did not surface their `rejectionReason`; it rendered the same notice as an unrecognized prompt.
...
- The prompt bar renders a command-not-executed receipt from `rejectionReason` before JSON-edit, action-execution, or no-match branches, and executes no action for that result.

## Consequences

Provider refusals and invalid structured responses cannot be bypassed through another provider, native text generation, or downstream action execution. Availability failures can still use the configured fallback chain. Users receive an explicit rejection receipt instead of a misleading no-match message.

...
The first executable LLM slice must admit only deterministic, bounded operations against an exact editable MIDI clip and must make their claimed undo behavior real before exposing them to providers.

## Decision

- Admit `quantizeNotes` and `transposeNotes` through the provider-neutral executable registry. Later packets must repair and admit the remaining transforms rather than inheriting this admission implicitly.
...
- Add an internal-only MIDI note restore action containing the complete pre-transform snapshot and expected post-transform snapshot. Undo restores only when the current notes still equal the expected post-transform state; concurrent divergence returns a conflict instead of overwriting later work. Redo reapplies the original deterministic transform.
...
WebLLM, future native inference, and arbitrary hosted providers can safely propose whole-clip quantize and transpose operations without a server. Their confirmation, CRDT commit, undo/redo, and macro behavior are real and testable; the remaining transforms stay unavailable until they meet the same contract.

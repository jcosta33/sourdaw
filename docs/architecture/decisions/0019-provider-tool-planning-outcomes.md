...
- WebLLM and native text responses map only one fully consumed, unambiguous JSON representation of an empty tool-call batch to `complete` with zero calls. Tool-call arguments must be plain objects. Empty text, refusal or other non-tool text, conflicting or trailing representations, malformed syntax, invalid calls, and truncated output map to `rejected` with a human-readable reason.

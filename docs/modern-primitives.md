...
(`cmdTypecheck`, `cmdLint`, `cmdValidate`, `cmdBuild`, `cmdTest`) through the repo's
`AGENTS.md` Commands table before relying on any swap — a stabilized primitive on
paper is still a build break if the project's version is below the floor.

...
(`pnpm typecheck`, `pnpm lint`, `pnpm deps:validate`, `pnpm build`, `pnpm test:run`)
before relying on any swap — a stabilized primitive on paper is still a build break if
the project's version is below the floor.

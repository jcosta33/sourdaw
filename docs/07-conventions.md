...
- Keep module-private constants at the top of the owning file. Add a dedicated constants file only when several files of one feature must agree on the values — not for a value one file uses. That dedicated file is a domain-named vocabulary file in the owning layer, never a module-root `constants.ts`: domain and wire vocabulary (protocol discriminants, param-id tables, format constants) lives in `models/`, service behaviour in `services/`, and view-only values in `presentations/` — because workers, engine, and use cases may import `models/` but nothing below them may import `presentations/`.

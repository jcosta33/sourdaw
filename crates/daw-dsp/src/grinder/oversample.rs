...
//! (`proof::oversample::Oversampler2x` — 15-tap Kaiser half-band, unity-gain
...
pub(crate) mod alias_probe {
    ...
    pub fn drive_tone(index: usize, sample_rate: f32, amplitude: f32) -> f32 {
        let phase =
            2.0 * std::f32::consts::PI * PROBE_FUNDAMENTAL_HZ * index as f32 / sample_rate;
        phase.sin() * amplitude
    }
}

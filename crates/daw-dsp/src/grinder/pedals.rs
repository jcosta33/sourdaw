...
impl OverdrivePedal {
    ...
    pub fn process_sample(&mut self, input: f32) -> f32 {
        ...
        let pre_hp_freq = 120.0 + self.drive * 460.0;
        ...
        let gain = 1.8 + self.drive * 14.5;
        let pushed = tightened * gain;

        self.sag_state += 0.0025 * (pushed.abs() - self.sag_state);
        let headroom = (1.05 - self.sag_state * (0.18 + self.drive * 0.22)).clamp(0.62, 1.0);
        let clipped = Self::soft_clip(pushed / headroom.max(0.35), self.drive) * headroom;

        let tone_freq = 850.0 + self.tone * 4_800.0;
        ...
        let voiced =
            self.tone_lp_state * (1.08 - self.tone * 0.18) + brightness * (0.24 + self.tone * 0.82);
        ...
        let level = 0.72 + self.level * 0.68;
        dc_trimmed * level
    }

    fn soft_clip(x: f32, drive: f32) -> f32 {
        let asymmetry = 0.08 + drive * 0.1;
        let positive = ((x + asymmetry) * (1.55 + drive * 0.45)).tanh();
        let negative = ((x - asymmetry) * (1.25 + drive * 0.3)).tanh();
        let blended = if x >= 0.0 { positive } else { negative };
        blended * (0.86 - drive * 0.08)
    }

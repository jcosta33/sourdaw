...
impl ThrustFilter {
    pub fn new(sample_rate: f32) -> Self {
        let mut f = Self { lp_state: 0.0, tilt_amount: 0.0, center_freq: 640.0, coeff: 0.0, sample_rate };
        f.update_coeff();
        f
    }
    ...
    pub fn set_mode(&mut self, mode: f32) {
        self.tilt_amount = match mode as u8 { 0 => 0.0, 1 => 0.5, _ => 1.0 };
    }

    #[inline]
    pub fn process(&mut self, x: f32) -> f32 {
        if self.tilt_amount < 0.001 { return x; }
...
impl SidechainEq {
    pub fn new(sample_rate: f32) -> Self {
        let mut eq = Self {
            ...
            enabled: false, freq: 1000.0, gain_db: 0.0, q: 1.0, sample_rate,
        };
        eq.update_coeffs();
        eq
    }

    pub fn set_freq(&mut self, freq: f32) { self.freq = freq.clamp(20.0, 20000.0); self.update_coeffs(); }
    pub fn set_gain(&mut self, gain_db: f32) { self.gain_db = gain_db.clamp(-18.0, 18.0); self.update_coeffs(); }
    pub fn set_q(&mut self, q: f32) { self.q = q.clamp(0.1, 10.0); self.update_coeffs(); }
    pub fn set_enabled(&mut self, enabled: bool) { self.enabled = enabled; }
    ...
    #[inline]
    pub fn process(&mut self, x: f32) -> f32 {
        if !self.enabled || self.gain_db.abs() < 0.01 { return x; }
        let y = self.b0 * x + self.b1 * self.x1 + self.b2 * self.x2
            - self.a1 * self.y1 - self.a2 * self.y2;
        self.x2 = self.x1; self.x1 = x;
        self.y2 = self.y1; self.y1 = y;

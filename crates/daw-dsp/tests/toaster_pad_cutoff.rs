...
//!
//! **What a rendered-response guard cannot see.** This file composes both
//! directions — `Pad::set_param` normalises, the filter re-expands — so it
//! catches the two mappings *disagreeing* and nothing else. Change the span
//! symmetrically, in both directions at once, and the error cancels in the round
//! trip: shrinking `CUTOFF_HZ_MAX` to 10 kHz leaves
//! `sixteen_levels_filter_mode_lands_each_grid_position_where_it_asked` fully
//! green, because every frequency it probes still round-trips exactly inside the
//! shrunk span. This file is the wrong instrument for that failure, and it is
//! only a partial one for the kit test above, which reds under a shrunk span via
//! *saturation* rather than via any sensitivity to the span itself.
//!
//! The span is guarded where the absolute numbers live: the decade marks in
//! `primitives::cutoff`, which assert that 200 Hz and 2 kHz sit at a third and
//! two thirds of travel. Those are single-direction, absolute assertions, and
//! they are what a symmetric span change actually reds. The two guard styles are
//! complementary, not ranked — neither subsumes the other.
//!
...
fn a_kit_cutoff_resonates_where_the_kit_asked_it_to() {
    ...
    // the old expansion put these at 7654.2 Hz, 10728.2 Hz and 14318.5 Hz
    // (+2.06%, +2.17% and +2.27% — 35.2, 37.2 and 38.9 cents).
...
fn a_fully_open_filter_corners_at_the_advertised_ceiling_not_above_it() {
    ...
    // 20480 Hz is not a stability problem — it sits under Nyquist even at
    // 44.1 kHz (22050 Hz), and prewarps to a finite positive `tan` of 8.90
    // against 20000's 6.80. It is simply not the frequency the range advertises.

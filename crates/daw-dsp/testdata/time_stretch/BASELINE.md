...
Encoding is raw frame-interleaved IEEE-754 `f32` little-endian. `manifest.json` records every sample rate, channel count, frame count, formula, role, and digest. The ignored `regenerate_time_stretch_fixtures` test is the sole generator; the ordinary manifest test regenerates the corpus in memory twice and requires both generations and every checked-in byte to match the manifest.

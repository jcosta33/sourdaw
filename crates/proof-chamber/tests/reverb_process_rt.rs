...
//! one is at fault.
//!
//! Guarded here: the four algorithms the product can select (plate, fdn-8,
//! fdn-16, spring), plus Reverse and Hybrid-in-its-default-mode, which are
//! compiled and dispatched but unreachable from any surface. Convolution — and
...
const ALGORITHM_CONVOLUTION: f32 = 4.0;
const ALGORITHM_HYBRID: f32 = 5.0;
...
fn guarded_run(instance: &mut ProofChamberInstance) -> Vec<f32> {
    ...
    let blocks: Vec<(Vec<f32>, Vec<f32>)> = (8..(8 + GUARDED_BLOCKS))
...
fn hybrid_process_does_not_allocate_in_its_default_algorithmic_only_mode() {
    let mut instance = configured(ALGORITHM_HYBRID);
    let out = guarded_run(&mut instance);
...
fn convolution_backed_engines_still_allocate_on_the_render_path() {
    ...
    if let Some(which) = std::env::var_os(CHILD_ENV) {
        ...
        let algorithm = if hybrid {
            ALGORITHM_HYBRID
        } else {
            ALGORITHM_CONVOLUTION
        };
        let mut instance = ProofChamberInstance::new(SAMPLE_RATE);
        instance.set_param("algorithm", algorithm);
        instance.set_param("mix", 0.5);

...
impl EngineHandle {
    ...
    /// stall until it resettles, and while it refills after a block larger
    /// than any the device had delivered, or a larger render callback. The
    /// figure cannot be published at open, because it follows the block size
    /// the device turns out to deliver and the slice the render callback turns
    /// out to ask for; the reader writes it each time it settles on a cadence,
    /// against a depth it has just observed, and retracts it whenever it stops
    /// serving.
    ///

...
function isUseCasesOrRepositoriesPath(filePath: string): boolean {
    ...
    return norm
        .split('/')
        .filter(Boolean)
        .some((s) => s === 'useCases' || s === 'repositories');
}
...
function resolveImportToAbsolute(importerPath: string, source: string): string {
    const absImporter = path.isAbsolute(importerPath) ? importerPath : path.resolve(process.cwd(), importerPath);
...
function rewriteRelativeSpecifiersInSource(_j: any, source: string, depthIncrease: number): string {
    ...
    return source.replace(/\bimport\s*\(\s*(['"])(\.\.?[^'"]*)\1\s*\)/g, (match, quote: string, spec: string) => {
...
export default function transform(fileInfo: FileInfo, api: API, options: Options) {
            ...
            decl.declarations.forEach((d: any) => {
                if (d.type === 'VariableDeclarator') {
                    if (d.id.type === 'Identifier') {
                        ...
                        j(d.id)
                            .find(j.Identifier)
                            .forEach((p) => names.push(p.node.name));
                    }
                }
            });
        }
        ...
        const findIdentifiers = (ast: any) => {
            ...
            const visitTsRefs = (n: any) => {
                ...
                for (const key of Object.keys(n)) {
                    if (key === 'loc' || key === 'start' || key === 'end' || key === 'tokens' || key === 'lines')
                        continue;
        ...
        imports.forEach((imp: any) => {
            ...
            if (neededSpecifiers.length > 0 || !imp.specifiers || imp.specifiers.length === 0) {
                ...
                const decl = j.importDeclaration(
                    neededSpecifiers.length > 0 ? neededSpecifiers : imp.specifiers,
                    j.literal(source)
                );
    ...
    for (const depsNode of sortNodesByDependency(
        Array.from(depsSharedNodes),
        `${path.basename(filePath)} → *Dependencies`
    )) {
    ...
    for (let index = 0; index < roots.length; index++) {
        ...
        const orderedBundle = sortNodesByDependency(
            [rootNode, ...myAuxiliaries],
            `${path.basename(filePath)} → ${expName}`
        );
        const newAstNodes = [
            ...reqImports,
            ...siblingImportsRoot,
    ...
    if (!useBarrel) {
        ...
        try {
            allFiles.forEach((importerPath) => {
                ...
                if (
                    rewriteViMocksForSplitTarget(j, rootAst, importerPath, filePath, exportLocationMap, appendSubPath)
                ) {
                    changed = true;
                }

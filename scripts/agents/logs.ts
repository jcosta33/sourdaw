...
function run() {
    ...
    if (!existsSync(logFile)) {
        ...
        console.log(
            dim(
                `Logs are only generated if the agent was launched after Swarm V3 update, and not using the 'current' backend.`
            )
        );
        process.exit(1);
    }
    ...
    if (lines.length > MAX_LINES) {
        console.log(yellow(`... (truncated ${lines.length - MAX_LINES} lines) ...`));

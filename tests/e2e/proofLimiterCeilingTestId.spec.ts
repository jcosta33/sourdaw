...
// Proof "Master limiter ceiling" knob: lives in the always-visible Check
// SideCard of the plugin rail. Uncovered by no prior E2E — this asserts the
// knob mounts as a role="slider" and responds to ArrowUp by raising
// aria-valuenow, the same contract deviceKnobKeyboardTestId asserts for Gluten.
test.describe('Proof master limiter ceiling — keyboard response', () => {
    ...
    test('ArrowUp increments aria-valuenow', async ({ page }) => {
        const ceiling = page.getByRole('slider', { name: 'Master limiter ceiling' }).first();
        await ceiling.focus();

...
acceptance conditions, integration observable, and exact checks, and, when the change carries a
handwritten path under product scope (`src/modules/`, `src/components/`, `electron/`), the
user-observable verification steps the pull request's How to test must teach. Before writing it, trace each
...
`assessmentImpact: none` outright, with or without a reason, and refuses any dossier with no
limitations regardless of impact: there is nothing to cite, so the round must disclose the gap as a
limitation instead. A bundle with no `semantic-ci.json` file at all — a historical bundle — still

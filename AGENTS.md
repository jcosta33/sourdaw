...
- **`.agents/worktrees/<name>/`** — isolated lanes for parallel work (gitignored). Create from `origin/main`; lock with `git worktree lock --reason active:<owner> <path>` while assigned. Operate only inside that lane. After its linked campaign closes and every process exits, unlock it and run `pnpm lane:remove <path>` elsewhere. Never remove lanes with raw Git.

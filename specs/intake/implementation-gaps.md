...
# Intake: Consolidated Implementation Gaps
...
This section extends §4 with items surfaced in `specs/plugin-hosting-clap/research.md` that were not previously covered.
...
- **Agent Tasks:**
    ...
    - Add a **time-stretch engine** with permissive licensing — options: `signalsmith-stretch` (if license-compatible), `tdpsola`, or a clean-room WSOLA/Phase-Vocoder — surfaced through a single `TimeStretch` trait and consumed by the clip launcher (`specs/non-linear-clip-launcher/spec.md`), warp operations, and offline bounce.
...
This section extends §6.2 with the full-lifecycle requirements surfaced in `specs/orchestra/research-global-harmonic-awareness.md`. It does not replace §6.2; it operationalises it.

...
> Co-located research for `specs/notation/spec.md`. The summarized findings (R-001…R-005)
> below are the working suspec; the **Restored research evidence base** at the foot of this
...
- [ ] Q-001 — DP quantizer cost-function weights (onset error vs notation complexity) need tuning against a human-transcription suspec.
...
grouping (R-005). Tune the DP weights (Q-001/Q-002) against a transcription suspec before
shipping defaults.
...
Transition probabilities between beat positions are learned from a score suspec. **Viterbi

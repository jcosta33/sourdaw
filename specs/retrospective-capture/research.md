---
type: research
id: RESEARCH-retrospective-capture
title: Always-on retrospective capture
status: open
owner: The Sourdaw team
sources:
  - "Question: how is always-on MIDI/audio retrospective capture implemented RT-safely, and how is tempo inferred?"
---

# Research: Always-on retrospective capture

## Question

How is always-on retrospective MIDI (and audio) capture implemented without harming the
audio thread, and how is a usable tempo and bar length inferred from a free performance?

## Findings

### R-001 — Lock-free SPSC ring buffer with an atomic write head

- **Claim:** A single-producer/single-consumer ring buffer with an `AtomicU64` write position lets the audio/MIDI thread push events without locks; the consumer reads a consistent snapshot via acquire/release ordering.
- **Evidence:** `rtrb` design; standard SPSC literature; `Ordering::Release`/`Acquire` pairing.
- **Confidence:** high
- **Bears on:** AC-001 (no audio-thread locks), AC-002 (snapshot consistency).

### R-002 — Locked memory avoids page faults in the buffer

- **Claim:** The capture buffer should be `mlock`/`VirtualLock`-pinned so the always-on writer never triggers a page fault on the realtime path.
- **Evidence:** JACK/realtime-audio guidance on memory locking; OS API parity.
- **Confidence:** medium
- **Bears on:** AC-001 and the buffer-sizing open question.

### R-003 — Tempo inference from an IOI histogram

- **Claim:** Inter-onset-interval histogramming plus autocorrelation recovers a beat period from a free MIDI performance; the dominant IOI cluster yields the tempo hypothesis.
- **Evidence:** beat-tracking literature (Dixon, Ellis); MIDI onset histogram methods.
- **Confidence:** medium
- **Bears on:** AC-004 (tempo inference).

### R-004 — Power-of-two bar snapping gives musical lengths

- **Claim:** Snapping the recovered capture length to the nearest power-of-two bar count (1, 2, 4, 8 bars) yields a loop-friendly clip rather than an arbitrary length.
- **Evidence:** common loop-length conventions; source design notes.
- **Confidence:** medium
- **Bears on:** AC-005 (bar-length snapping).

### R-005 — Orphaned note-on recovery

- **Claim:** When capture begins mid-note, a note-on with no matching note-off in the window must be reconstructed (synthesised note-off at window end or carried duration) so the clip has no hung notes.
- **Evidence:** MIDI capture edge-case analysis in the source.
- **Confidence:** medium
- **Bears on:** AC-003 (orphaned-note recovery).

## Open questions

- [ ] Q-001 — Default buffer duration (e.g. 5 vs 10 minutes) and per-event memory cost vs the mlock budget.
- [ ] Q-002 — Confidence threshold below which inferred tempo is discarded in favour of the project tempo.
- [ ] Q-003 — Should always-on audio capture share the MIDI buffer's lifecycle or be opt-in per track?

## Recommendation

Use a single SPSC ring buffer with an `AtomicU64` head (R-001) on pinned memory (R-002),
recover tempo from an IOI histogram (R-003) and snap to power-of-two bar lengths (R-004),
and reconstruct orphaned note-ons on extraction (R-005). Resolve the buffer-sizing and
tempo-confidence questions (Q-001/Q-002) before shipping defaults.

---

# Restored from research/features/retrospective-capture.md

> The following sections are the original long-form depth research for this feature
> (`research/features/retrospective-capture.md`), restored verbatim into this
> co-located research doc after a migration dropped them. The condensed R-001…R-005
> findings above summarise these; the detail below is the authoritative source.

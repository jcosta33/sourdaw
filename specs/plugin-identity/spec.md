...
Verify with: `manual` — confirm Fermenter is sage (not mint), Dutch Oven is amber and Crust is peach (not the same copper), Yeast is rose (not Levain's coral), and Scoring is indigo (not Proof's cyan); the resolved assignments are recorded in `specs/plugin-identity/research.md#color-separation-analysis-recovered`
...
- Re-embedding the full per-plugin hex palette and CSS module templates — those live
  ...
  `specs/plugin-identity/research.md#plugin-color-matrix-recovered`.
- The measured Quick Reference Contrast Ratios table (11 colors on `#111111` and
  ...
  in `specs/plugin-identity/research.md#quick-reference-contrast-ratios-recovered`.
- The energy-level glow-opacity tuning table — an implementation nicety, not a
  verifiable requirement; preserved in
  `specs/plugin-identity/research.md#glow-intensity--energy-level-table-recovered`.

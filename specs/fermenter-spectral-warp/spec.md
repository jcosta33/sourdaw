...
- Dangling reference: the original implementation notes pointed implementers at
  the pre-Suspec Fermenter implemented spec (no longer in the suspec; preserved in
  ...
  current suspec, so the math reference it cited is gone. The per-mode effect
  ...
  cited pre-Suspec Fermenter implemented spec (no longer in the suspec; preserved

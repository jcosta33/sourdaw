...
Verify with: `cargo test -p daw-engine sampler`
...
Verify with: `cargo test -p daw-engine voice_allocator`
...
Verify with: `cargo test -p daw-engine voice_stealing`
...
Verify with: `cargo test -p daw-engine streaming`
...
Verify with: `cargo test -p daw-dsp pitch` — an A4 sine resolves to 440 Hz / MIDI 69 with
...
Verify with: `cargo test -p daw-dsp onset`
...
Verify with: `cargo test -p daw-engine round_robin`
...
Verify with: `cargo test -p daw-engine velocity_layers`
...
Verify with: `cargo test -p daw-engine articulators` — 8-slot pad processes 10,000 note-ons with `assert_no_alloc` reporting zero RT allocations
...
Verify with: `cargo test -p daw-engine mod_matrix` — 32 routings evaluate in bounded time/block; adding a 33rd is rejected at config time with a UI error
...
Verify with: `cargo test -p daw-engine mod_xy` — modX 0.0/0.5/1.0 → A / −3 dB both / B within ±0.5 dB
...
Verify with: `cargo test -p daw-engine flex_speed` — LFO→warp-ratio over 1,000 cycles with `assert_no_alloc` and pre/post FFT-buffer-capacity check
...
Verify with: `cargo test -p daw-engine process_callback` — full orchestration runs under `assert_no_alloc` in debug builds
...
Verify with: `cargo test -p daw-engine recorder` — threshold-triggered capture starts on signal crossing the threshold and joins gaplessly
...
Verify with: `cargo test -p daw-dsp smart_loop` — detected loop points land on zero-crossings and the boundary crossfade is equal-power
...
Verify with: `cargo test -p daw-engine constants` — each constant resolves to its specified value
...
Verify with: `cargo test -p daw-engine round_robin_modes` — RandomNoRepeat over 1,000 triggers has no adjacent repeats; reset returns to S0; `assert_no_alloc` over 10,000 triggers
...
Verify with: `cargo test -p daw-dsp ahdsr` — state transitions, target_ratio curve shape, and each retrigger mode behave as specified
...
Verify with: `cargo test -p daw-dsp svf` — all six outputs are present and 2× oversampling engages for Q > 10

### AC-025 — Velocity layering supports up to 16 layers

Each pad/slice must support up to 16 velocity layers.

Verify with: `cargo test -p daw-engine velocity_coverage` — a `[10..20],[30..40]` config is rejected naming the uncovered ranges; 16-layer config resolves deterministically
...
Verify with: `cargo test -p daw-engine voice_stacking` — `stack_count=4, detune=20` yields offsets ≈ {−20, −6.67, +6.67, +20} cents (±2 cents by FFT)
...
Verify with: `cargo test -p daw-engine mpe` — channels 2–8 carrying distinct note-ons route to distinct voice slots and their per-note pitch-bend source modulates pitch
...
Verify with: `cargo test -p daw-engine granular_warp` — each window shape, jitter bound, and the COLA continuity property hold
...
Verify with: `cargo test -p daw-engine streaming_io` — 4096-sample chunk reads, priority ordering by remaining buffer, and non-fatal 256-entry overflow reporting
...
Verify with: `cargo test -p daw-dsp onset_picking` — peak-pick windows, ZC snap range, gating intervals, and the sensitivity→threshold mapping are applied

### AC-031 — Voice structs are cache-line aligned

Each voice struct must be 64-byte cache-line aligned to avoid false sharing.

Verify with: `cargo test -p daw-engine voice_layout` — `size_of`/alignment is 64-byte aligned; steal fade is linear 1–5 ms and choke fade is 3 ms raised-cosine
...
Verify with: `cargo test -p daw-engine velocity_coverage` — a `[10..20],[30..40]` config is rejected naming the uncovered ranges
...
Verify with: `cargo test -p daw-engine voice_layout` — allocation uses `compare_exchange_weak` with `AcqRel` ordering
...
Verify with: `cargo test -p daw-engine voice_layout` — steal fade is linear 1–5 ms and choke fade is 3 ms raised-cosine
...
Verify with: `cargo test -p daw-dsp beat_grid` — estimated beat positions are exposed and a slice
...
Verify with: `cargo test -p daw-dsp bpm` — a clip of known tempo resolves to its BPM via GAC
...
Verify with: `cargo test -p daw-dsp waveform_mipmap` — level 0 reduces a 32/64-sample base block to a
...
Verify with: `cargo test -p daw-dsp src_skip` — a sample loaded in repitch mode in a mismatched-rate

...
Verify with: `cargo test -p daw-io scala_scl_fixture_conformance`
...
Verify with: `cargo test -p daw-io kbm_roundtrip`
...
Verify with: `cargo test -p daw-io ascl_live12_roundtrip`
...
Verify with: `cargo test -p daw-io scl_write_roundtrip`

### AC-005 — Malformed input never panics

A missing count line, non-numeric ratio, or out-of-range reference note must yield a
`TuningParseError` naming the line; a `cargo fuzz` target runs ≥5 minutes in CI without a
panic.

Verify with: `cargo test -p daw-io scala_parse_errors`
...
- `crates/daw-io/` (Scala parsers/writers), CI fuzz target

...
# Intake: Consolidated Audit — Deferred Fixes

---
name: consolidated-audit-deferred-fixes
description: Implementation spec for deferred items from the historical inventory overview. Each section is independently implementable with verifiable acceptance criteria.
type: spec
status: open
related-audit: historical inventory overview (not retained)
---
...
The historical cross-module inventory overview was partially addressed in the 2026-04-16 fix pass and was not retained after migration. Thirteen issues are fixed in code. The remaining issues are deferred either because they are architectural (require a contract/interface), require performance work that needs care (Rust DSP, RT paths), require a UX or product decision, or require a small spec to specify behaviour beyond a one-line code change.

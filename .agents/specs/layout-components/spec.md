...
Verify with: `manual` — walk the migration-examples mapping in `.agents/specs/layout-components/audit.md` and confirm each common audited pattern (`flex flex-col gap-*`, `flex items-center gap-*`, `grid grid-cols-* gap-*`, `space-y-*`) resolves to a primitive
...
Present-state findings from the layout audit (`.agents/specs/layout-components/audit.md`).

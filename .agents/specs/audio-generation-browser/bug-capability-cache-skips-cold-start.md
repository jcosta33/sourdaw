...
```text
.agents/specs/audio-generation-browser/spec.md:229:### AC-025 — Capability detection re-runs on every cold start with provenance
.agents/specs/audio-generation-browser/spec.md:231:Capability detection must run on every cold start (not only first launch) so a WebView2/WebGPU
.agents/specs/audio-generation-browser/spec.md:234:Verify with: `pnpm test:run -- BrowserAi capabilityColdStart`
```

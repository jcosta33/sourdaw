...
- [ ] (non-blocking) (deferred-gap from intake/implementation-gaps.md §5.1 — Integrated Stem Separation Workflow) Demucs already runs locally, but stem separation is not integrated into the workflow UI. Three distinct behaviors are needed: (a) **drag-and-split** — a drag gesture on a clip that triggers Demucs separation; (b) **auto-route separated stems to dedicated mixer lanes** — each separated stem (e.g. vocals, drums, bass, other) lands on its own dedicated mixer lane automatically; (c) **easy re-sampling of stems into the sampler suite** — a path to load a separated stem directly into the sampler instruments. This is forward, multi-part scope spanning clip interaction, mixer routing, and sampler ingestion (largely outside this spec's MIDI-editor focus); it likely warrants its own dedicated spec rather than ACs here.

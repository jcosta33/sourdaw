...
Status: FIXED in #747 — collapsed the three curve-value evaluators that feed playback and the editor readout onto one shared kernel (`src/utils/automationCurve.ts`, `evaluateAutomationCurve`): the live apply path (`interpolateAutomationPointValue`), the offline compile path (`compileAutomationEvents`), and the editor playhead readout (`Arrangement/transformers/automationTransformers.ts` `interpolateAutomationValue`, which also gained the missing `bezier` branch and the `stairs` clamp). Convergences: live adopted the offline-aligned `stairs` clamp [2,32] + fraction clamp; offline adopted the absent-tension default; the editor readout adopted both plus real `bezier`. A permanent cross-conformance gate (live-side, offline-side, and editor-readout `automationCurveConformance` specs, sharing one case table) now guards re-divergence. Behavioral caveat: a project whose points carry out-of-range `stairSteps` (0 / negative / fractional / >32) now clamps in **live playback and the editor readout** too — previously they were played/shown unclamped (0 → `NaN`, >32 → finer steps), disagreeing with the bounce; they now match it. Residual (follow-up, not this PR): one non-playback evaluator remains independent — TimelineEditor's `buildCurvePath` (`TimelineEditor/presentations/helpers/automationViewHelpers.ts`) computes the automation curve as an SVG path for rendering; it is a rendering-layer copy (emits path geometry, not a scalar value) and should be reconciled with the kernel's control-point handling in a follow-up.

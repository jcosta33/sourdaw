...
_Note: This document consolidates research on UI/UX, Automation, and the Sample Library. Features that have already been fully implemented in the Sourdaw codebase have been removed. The remaining sections detail missing capabilities or architectural plans that still need to be built, annotated with findings from the current codebase._
...
_Implementation planned:_ CSS `conic-gradient` with `--mod-amount` custom property updated from JS at 30fps. GPU-composited by browser.
...
- **Spectrogram (waterfall):** Frequency on Y-axis, time on X-axis, amplitude as heatmap color. iZotope RX gold standard — waveform + spectrogram overlay.
  _Implementation planned:_ WebGPU. Upload FFT data as Float32Array to GPU storage buffer each frame.

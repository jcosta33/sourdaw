...
> **Codebase Annotation:** Native plugin hosting currently relies on a custom `Vst3Wrapper` (`src/host/vst3_wrapper.rs`) using `libloading`, diverging from the recommended CLAP integration via `clack-host`. Advanced features like out-of-process sandboxing (via `shmem-ipc`), `creek` based disk streaming, and native Web Audio node offloading are **Missing**.

# Articulation maps and orchestral intelligence for a Tauri v2 DAW

**Articulation maps are the single most requested—and most poorly implemented—feature across commercial DAWs.** Every major implementation (Logic Pro, Cubase, Studio One) suffers from tedious setup, buggy chase behavior, and visual feedback that breaks down at scale. The opportunity for a Tauri v2 DAW with a Rust real-time engine and CRDT state is to leapfrog these systems by combining Cubase's Direction/Attribute duality and unlimited output mappings with Reaticulate's program-change-based chase elegance—while solving the UX disasters that have persisted since Steinberg introduced Expression Maps in 2009. This document covers every layer: editor UX, piano roll integration, data model (TypeScript + Rust), real-time MIDI interception architecture, chase algorithms, and the edge cases that trip up existing implementations.
...
The core architectural insight is that **articulations should be note metadata** (following Logic's model) for editing reliability, but **projected as program changes or keyswitch events** at playback time (following Reaticulate's model) for native chase support. This separation of the semantic layer (what the user sees and edits) from the MIDI layer (what the instrument receives) is what no existing DAW fully achieves.

The `ArcSwap` pattern for lock-free map sharing, combined with pre-computed chase snapshots at bar boundaries and per-parameter binary search indexes, provides a real-time-safe architecture that avoids the chase bugs plaguing Cubase for over 15 years. The pre-processing stage between event collection and plugin dispatch is the correct insertion point—it maintains sample-accurate timing while keeping the articulation system transparent to plugins.

For UX, the three missing features that would differentiate this DAW are: **inline articulation labels on piano roll notes** (no DAW does this), **per-articulation timing compensation** (only Cubase 15 has this), and **automated map creation** via plugin introspection (only ArtiQverse and Studio One's dynamic API attempt this). Combined with a CRDT-native data model that enables real-time collaborative editing of articulation maps, this architecture addresses every major pain point identified in the orchestral composer community.

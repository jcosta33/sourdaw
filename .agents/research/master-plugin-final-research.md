...
_Annotation: The frontend `src/modules/Fermenter/presentations/` implements `MacroStrip` and `LayerStack`, but lacks the full 5-level progressive disclosure and the central `ContextInspector`._

- **Progressive Disclosure Levels:**
    - _Level 1 - Play:_ Preset browser, macro strip. (Implemented via `MacroStrip`).
    - _Level 2 - Shape:_ Layer stack, basic generator/filter/envelope controls. (Implemented via `LayerStack`).
    - _Level 3 - Build:_ Full layer stack, per-layer mixer, full modulation dock. (Missing explicit Build view).
    - _Level 4 - Route:_ Lane routing map, serial/parallel/split toggles, per-voice FX placement. (Missing explicit Routing view).
    - _Level 5 - Lab:_ Import/analysis workflows, wavetable editor, additive editor, granular source editor. (Missing Lab view).

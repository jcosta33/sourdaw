...
Not every spec runs this way: one that resolves the checkout's own tooling or a dependency through its own file location or through `process.cwd()/node_modules` cannot, because the worktree has no install. Recognize the class in the spec or its diff: it builds a path to `node_modules`, or to a binary inside it, from its own file location or from `process.cwd()`, then executes or imports through it — its swallowed ENOENT and exit 1 look exactly like a real head regression, so classify from the spec, never from the artifact. A second class needs the package-manager environment: a spec that resolves the package manager through `process.env.npm_execpath` and requires that variable to point at pnpm. With the variable unset it stops on an explicit missing-prerequisite message; with it naming another package manager — the ambient value under some launching harnesses — it stops on that manager's own command error, which looks like an ordinary failure while being an environment failure. Neither shape is evidence about the head. Treat a resulting environment failure as a limitation to record and verify by reading the head — never as evidence about the head. Record the limitation and report the head's corresponding claim as unverified, naming the spec, so the orchestrator can run it where the install exists; the mandatory mutation-and-named-spec baseline probe is undischargeable for a spec the round could not run, so the round must say so rather than pass; a reviewer who can run such a spec by pointing the variable at a real package manager does so under the no-symlink and no-install rules above and records the environment it used.

...
Not every spec runs this way: one that resolves the checkout's own tooling or a dependency through its own file location or through `process.cwd()/node_modules` cannot, because the worktree has no install. Treat a resulting environment failure as a limitation to record and verify by reading the head — never as evidence about the head.

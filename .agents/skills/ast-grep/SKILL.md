...
- Use the full command name `ast-grep`, never `sg`: the alias is deprecated by the package itself (its shim prints a deprecation warning), collides with another executable on Linux hosts, and is denied outright by Claude Code permissions in every spelling the skill and repository teach (direct, `pnpm`, `pnpm exec`, `npx`, `npx -y`, and `bin` paths).

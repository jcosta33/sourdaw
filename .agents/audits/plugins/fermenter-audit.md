...
1. [FIXED] **Per-Block Audio Thread Allocation in `layer.rs` (`Layer::render`)**:
...
3. [FIXED] **Per-Block Allocation in WASM interop (`mod.rs`)**:
...
1. [WRONG] **Sample-Rate Dependent Voice Stealing (`voice.rs`)**:

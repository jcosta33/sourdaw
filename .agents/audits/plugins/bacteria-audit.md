...
1. **Catastrophic Per-Sample Allocation in Oversampling (`engine.rs`)**:
   *   **Issue:** Inside `BandChain::process_sample`, if the `oversampling_factor` is greater than 1, the code upsamples a *single sample*, allocates a new `Vec` to hold the oversampled data (`let mut processed_l = vec![0.0_f32; up_l.len()];`), processes it, and downsamples it.
...
1. **Sample-Rate Dependent Metering (`engine.rs`)**:
...
### 🐛 Logical Bugs
1. **Unused Modulation System (`engine.rs`)**:
   *   **Issue:** The engine does the mathematical work of advancing complex modulation sources (LFOs, Envelope Follower, Lorenz Attractor, Step Sequencer) and saving them into `mod_values`. It also allows registering `ModAssignment` and `MacroMapping`. However, **it never actually applies these modulations** to the effect parameters anywhere in `process_block`.
   *   **Impact:** The entire modulation matrix and macro mapping system is effectively a no-op; tweaking macros or assigning LFOs to parameters will do absolutely nothing to the audio.

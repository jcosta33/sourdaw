...
1. [FIXED] **Per-Block Allocation in WASM interop (`mod.rs`)**:
...
1. [FIXED] **Missing Audio Input Binding (`mod.rs`)**:
...
2. [FIXED] **Block-Size vs. Pitch Tracking Window Mismatch (`yin.rs` & `engine.rs`)**:
...
3. [x] **Missing DSP implementation (`engine.rs`)**:
    - **Issue:** The plugin is described as a "Real-time pitch manipulation" engine, and there is a `psola.rs` module in the directory. However, `engine.rs` only runs the YIN pitch _analysis_. It never imports, instantiates, or executes any pitch-shifting or PSOLA code.

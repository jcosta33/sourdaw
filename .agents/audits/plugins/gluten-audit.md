...
1. [x] **Broken Auto-Makeup Gain (`engine.rs`)**:
    ...
    - **Fix:** The `compute_auto_makeup` method must read the _actual current_ threshold and ratio parameters from the active topology instance (e.g., `self.vca.get_threshold()`) instead of using hardcoded literals.

2. [FIXED] **Destructive Mid/Side Mode (`engine.rs`)**:

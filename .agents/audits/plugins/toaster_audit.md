...
- **Worse yet:** `FermenterNode` and `LevainNode` suffer from _the exact same bug_. They fetch `/wasm/fermenter/fermenter_bg.wasm`.
...
Once we fix the WASM pipeline (Step 1), the new DSP code _will_ load... but it will immediately break across several drum models because the Rust parameters are mapped destructively against UI macros:

### [FIXED] The Snare (Sounds like a Kick Drum)
...
### [FIXED] The Hi-Hat (Thin and Missing Harmonic Ratios)
...
### [FIXED] Cowbell & Shaker (Missing Noise Exciter)
...
1. [FIXED] **Pipeline Restructuring (`ts` bridge)**

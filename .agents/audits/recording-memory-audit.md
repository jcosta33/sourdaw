...
5. **Synchronous IndexedDB Mass-Loading (`audioBufferCache.ts`)**
   ...
   *   **Fix:** Load audio files lazily (on-demand) just-in-time for playback, or stream them directly from OPFS without loading the entire file into a single `AudioBuffer`.
   > ⬜ **Code-verified:** Confirmed real bug. `restoreFromIdb()` loads all keys sequentially via `store.getAllKeys()` then fetches each one. **Deferred — proper fix (lazy on-demand loading + OPFS streaming) is part of the same audio pipeline architectural rewrite. The LRU cap (item 4) bounds peak RAM in the meantime; mass-loading into a capped store is less catastrophic than mass-loading into an unbounded one.**

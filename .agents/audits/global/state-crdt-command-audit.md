...
1. **Unthrottled Synchronous CRDT Writes (`AutomergeStorage.ts`)**:
    ...
    - **Fix:** Continuous/ephemeral UI interactions must be decoupled from CRDT persistence. High-frequency updates should mutate the local, in-memory cache _only_. CRDT mutations should either be debounced (e.g., waiting 100ms after the last change) or only triggered on `pointerup` / `dragend` events.
...
2. **Synchronous Network Merging (`crdtMerge.ts` & `automergeRepository.ts`)**:
    ...
    - **Fix:** CRDT merging and loading must be offloaded to a Web Worker. The worker should perform the `Automerge.merge` operation in the background and only post back the resulting lightweight JavaScript object state to the main thread for React to render.
        > ⬜ **Code-verified:** Confirmed real bug. `automergeRepository.ts` `loadAll()` calls `Automerge.load()` for each base doc and `Automerge.loadIncremental()` for each incremental chunk synchronously on the main thread. `mergeBundle()` calls `Automerge.load()` + `Automerge.merge()` per document, also synchronously. Loading a large project (many incremental chunks) or merging a large collaboration patch will freeze the UI. **Deferred — moving Automerge merge/load into a Worker requires the Automerge WASM binary to be transferred (or re-initialized) in the Worker context, plus a full message-passing protocol for all store hydration. This is the most disruptive architectural change in the codebase; it touches every store and the CRDT sync layer simultaneously. Cannot be incrementally patched — requires its own dedicated workstream.**
...
1. **Synchronous Hydration Diffing (`AutomergeStorage.ts`)**:
    ...
    - **Fix:** Automerge provides native delta updates. The projection layer should rely on Automerge's change patches or shallow equality checks rather than deep JSON string comparisons.
        > ✅ **FIXED (partial):** `hydrate()` now computes `incomingJson = JSON.stringify(value)` once and reuses it for both `JSON.parse` (replacing the `#toDocSafe` round-trip) and the change-detection comparison on the non-merge path. The non-merge path no longer re-stringifies the cache after assignment — it compares `incomingJson !== beforeJson` directly, saving one full `JSON.stringify` per hydration call. The merge path (stores with ephemeral fields) still requires one extra stringify for the merged object. Full Automerge patch-based diffing remains deferred alongside the Worker workstream.

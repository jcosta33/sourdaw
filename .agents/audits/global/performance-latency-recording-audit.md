...
- The UI thread should _never_ calculate audio math.
...
> ✅ **FIXED (allocation):** `new Float32Array(analyser.frequencyBinCount)` per frame eliminated. `LUFSMeter.tsx` now allocates a single `analyserData` buffer in `useEffect` scope and reuses it every rAF tick, reallocating only when `frequencyBinCount` changes. The K-weighting approximation in `computeMomentaryLUFS` remains (acknowledged in `lufs.ts`); the SharedArrayBuffer/Worklet proposal is still the production-grade fix when accuracy matters.

...
1. **The "React-Coupled Canvas" Anti-Pattern (`PianoRoll.tsx` & `usePianoRollRenderer.ts`)**:
    ...
    - **Impact:** When a user clicks and drags a MIDI note, `moveMidiNote` is called continuously (`mousemove`). This mutates the global store. React then intercepts this, forces the entire `PianoRoll` component to re-render, reconciles the React tree, and _then_ fires the `useEffect` to redraw the canvas. This guarantees that dragging a note will lag, stutter, and drop frames, as React is simply not fast enough to sit between a `mousemove` event and a Canvas repaint at 120Hz.
    - **Fix:** The Piano Roll must adopt the `TimelineSurface` architecture. Canvas drawing must be completely detached from React renders. Note dragging should mutate an ephemeral, non-reactive Ref or class property, and an `AnimationScheduler` should pull from that Ref to draw the canvas. Global state (`midiStore`) should only be committed on `mouseup`.
        > ⬜ **Code-verified:** Confirmed real bug. Needs architectural refactor — PianoRoll rendering must be decoupled from React lifecycle. **Deferred — rewriting the Piano Roll to use an `AnimationScheduler` + ephemeral drag ref (mirroring `TimelineSurface`) is a substantial isolated effort. The most impactful drag-related issue (global CRDT write on every `mousemove`) was already fixed in item 3. The remaining React-coupled repaint is a frame-pacing issue rather than a data correctness bug; usable but not optimal. Requires its own dedicated refactor.**
...
2. **Excessive Canvas 2D Overdraw (`usePianoRollRenderer.ts`)**:
    ...
    - **Fix:** 1. **Layering:** The grid and ruler must be drawn _once_ to an `OffscreenCanvas` (or separate DOM `<canvas>`) which is only redrawn on zoom/scroll. The foreground interactive notes should be drawn on a separate transparent canvas layer. 2. **WebGPU Migration:** Just like the main timeline, the Piano Roll should migrate its dense drawing paths to the `createWebGpuRenderer` infrastructure to offload the thousands of rectangles to the GPU.
        > ⬜ **Code-verified:** Confirmed real architectural issue. Coupled to fix #1 — decoupling from React lifecycle is the prerequisite. **Deferred — blocked on item #1. Once the Piano Roll adopts `AnimationScheduler`, the `OffscreenCanvas` grid layer and WebGPU migration become straightforward incremental additions. No point layering drawing optimisations onto a renderer that still re-enters the React lifecycle.**
...
3. **Global State Mutation on Mouse Move (`usePianoRollInteractions.ts`)**:
    ...
    - **Fix:** `handleMouseMove` must update an ephemeral `dragPreviewRef` (similar to what it does for drawing _new_ notes) and only commit the actual `moveMidiNote` CRDT mutation inside `handleMouseUp`.
        > ✅ **FIXED:** `usePianoRollInteractions.ts` now uses an ephemeral `dragPreviewRef` during note move/resize. `handleMouseMove` no longer calls `moveMidiNote`/`resizeMidiNote` — it updates the ref and calls `draw()` directly. `handleMouseUp` commits the final delta with a single `moveMidiNote`/`resizeMidiNote` call per note, then pushes the undo entry. `usePianoRollRenderer.ts` reads `dragPreviewRef.current` to apply visual offsets without touching the store. Dragging notes now produces 0 `midiStore.set()` calls instead of 60/s.

...
2. **Zipper Noise & Message Flooding in Automation (`applyAutomation.ts`)**:
    ...
    - **Fix:** Automation should utilize the Web Audio API's native scheduling curves (`AudioParam.setValueCurveAtTime` or `linearRampToValueAtTime`) passing an array of future points, or the worklet should implement parameter smoothing (which some Rust plugins do, but native GainNodes need Web Audio ramps).
        ...
        > ✅ **FIXED (plugin params):** `applyAutomation.ts` now applies a single-pole IIR slew (α = 0.4, ~90ms to 95% of target at 100Hz) for all plugin device parameters before calling `updateDeviceParam`. The smoothed value is cached in `_pluginParamSlew` (keyed by `trackId:deviceId:paramId`). When the smoothed delta falls below `5e-5` (flat curve), the `postMessage` dispatch is skipped entirely — eliminating redundant flooding on stable automation lanes. Faust plugin params already receive audio-thread smoothing via `setTargetAtTime`; the main-thread pre-slew feeds them a smoother ramp so both smoothers work in the same direction. Full WASM-side `AudioParam`-mapped smoothing remains the production-grade fix.

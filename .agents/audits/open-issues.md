...
**Last verified:** 2026-04-07 — **codebase pass:** `pnpm deps:validate` (0 violations, 1553 modules) and `pnpm typecheck` (pass). **Second pass (same day):** spot-read stores + refs for items 4, 10, 19, 21 — see verification table rows marked _Recheck_.
...
| _Recheck_ · Item 10 (volatile routing/CV)                      | **Partially outdated** — `sidechainStore` and `cvGateStore` use **`createAutomergeStorage`** (`sidechainRoutes`, `cvGate` keys on root doc). Still **legit** for **`kneadStore`** (no storage) and **`actionHistoryStore`** (no storage).                                                                                                                                  |
| _Recheck_ · Item 4 (singleton stores)                          | **Nuance** — `fermenterStore` / `bacteriaStore` are **`Record<deviceId>`**, not single global rows. Item stays as **document persistence / CRDT** alignment for plugin state, not “two Fermenters clobber one store” in the old sense.                                                                                                                                     |
| _Recheck_ · Item 19 (SampleLibrary path)                       | **`connectFolderTauri`** is a **nested function** in `src/modules/SampleLibrary/useCases/connectFolder.ts`, not a standalone `connectFolderTauri.ts` file — fix refs.                                                                                                                                                                                                      |
| _Recheck_ · Item 21 (PianoRoll)                                | **`PianoRoll.tsx`** uses **`useStore`** from `#/infra/store/useStore` for store subscriptions — same perf concern, wording fixed in item 21.                                                                                                                                                                                                                               |

...
> ⚠️ **Note:** The implementation has already moved away from `MediaRecorder` to `ScriptProcessorNode` for raw PCM capture (recorded in `recording-memory-audit.md`). This fixes the lossy compression issue but `ScriptProcessorNode` is itself deprecated and runs on the main thread. The issues around jitter and main-thread execution still apply — see `recording-memory-audit.md` for the detailed breakdown.
...
> ⚠️ **Code-verified:** Lighter than originally feared. `computeMomentaryLUFS` is a simple ~1024-element RMS loop (first-order highpass approximation, not a true biquad K-weighting filter), running at 60Hz. React state is already throttled to 10fps via `STATE_UPDATE_INTERVAL`. The analysis loop allocates a `new Float32Array(analyser.frequencyBinCount)` per frame, which adds mild GC pressure. The TODO comment in `lufs.ts` already acknowledges the K-weighting approximation. Not a blocking concern, but the SharedArrayBuffer/Worklet approach in the proposal would be the correct production-grade fix when accuracy matters.
...
> ✅ **Confirmed positive.** `playheadPositionRef` pattern is correctly implemented and in use.
...
> ✅ **Confirmed positive.** The waveform cache LRU cap (256 entries) added in `audioBufferCache.ts` partially addresses this for the CPU-side peak cache.

...
1. **Jittery MIDI Event Scheduling via `setTimeout` (`scheduleMidiNotes.ts`)**:
   ...
   *   **Fix:** The Rust AudioWorklet processors must implement a lock-free event queue (via `SharedArrayBuffer` or a WASM-side queue). The main thread should push timestamped MIDI events into this queue in advance, and the `process` loop in the AudioWorklet should pull and trigger them at the exact sample frame.
   > ⬜ **Code-verified:** Confirmed real bug. `scheduleMidiNotes.ts` uses `setTimeout` for Fermenter (lines 316–325), Levain (lines 333–349), and Toaster (lines 279–282) plugins. Architectural fix needed — requires SharedArrayBuffer event queue in AudioWorklet. **Deferred — implementing a lock-free MIDI event queue in the Rust AudioWorklet processors requires changes to every instrument's WASM interface, the JS-side `postMessage` protocol, and the scheduling loop. This is the largest single performance bug in the scheduling layer but is inseparable from the plugin architecture. Cannot be safely done incrementally; requires a dedicated MIDI-scheduling workstream.**
...
2. **Zipper Noise & Message Flooding in Automation (`applyAutomation.ts`)**:
   ...
   *   **Fix:** Automation should utilize the Web Audio API's native scheduling curves (`AudioParam.setValueCurveAtTime` or `linearRampToValueAtTime`) passing an array of future points, or the worklet should implement parameter smoothing (which some Rust plugins do, but native GainNodes need Web Audio ramps).
   ...
   > ⬜ **Still open (plugin params):** `updateDeviceParam` dispatches raw values via `postMessage` to AudioWorklets without smoothing. Plugin parameter automation (e.g., filter cutoff, reverb mix) is still susceptible to zipper noise at 100Hz. Plugin-side smoothing or Web Audio `AudioParam` ramps are required for those paths. **Deferred — adding per-parameter smoothing inside each Rust DSP processor requires plugin API changes (a smoothed-value primitive or an `AudioParam`-mapped interface). This needs coordination with the WASM plugin interface design, which is also touched by the MIDI scheduling fix above. Tracking as a known audio quality issue; lower severity than the MIDI jitter bug.**

...
- Where Tauri types are the behaviour — window creation, event emission, the raw-body invoke shape — the shell implements the corresponding `sourdaw-native` trait (`events.rs`, `windows.rs`, `commands/binary_ipc.rs`) instead of pushing the Tauri type inward.
...
The audio-thread rules are stated once, in `crates/sourdaw-native/AGENTS.md`, and they bind this shell too: nothing reached from the CPAL callback may allocate, lock or emit, and the window and event implementations here are forbidden on that path.

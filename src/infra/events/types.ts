...
export type EventBus<TEvents extends EventMap> = {
    on<TEventName extends keyof TEvents & string>(event: TEventName, handler: EventHandler<TEvents[TEventName]>): () => void;
    once<TEventName extends keyof TEvents & string>(event: TEventName, handler: EventHandler<TEvents[TEventName]>): () => void;

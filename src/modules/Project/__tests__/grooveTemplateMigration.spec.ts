...
describe('groove template project migration', () => {
    beforeEach(() => hydrateGrooveTemplates(structuredClone(defaultGrooveTemplateState)));
    ...
    it('preserves every legacy Yeast processor groove selection as a scoped canonical assignment', () => {
        ...
        for (const [templateIndex, templateName] of legacyTemplateNames.entries()) {
            const assignment: unknown = migrated.grooves.assignments.find(
                (candidate: unknown) =>
                    isRecord(candidate) &&
            ...
            const template: unknown = migrated.grooves.templates.find(
            ...
            for (const [index, timingOffset] of legacyTimingOffsets[templateIndex]!.entries()) {
                if (timingOffset === 0) {
                    continue;
                }
                slots.push({ index, timingOffset, dynamicsOffset: 0 });
            }

...
import { renderOffline, exportStems, downloadWav, downloadMp3, downloadFlac } from "#/modules/AudioEngine/useCases/offlineRender";
...
export const ExportDialog = ({ open, onClose }: ExportDialogProps): ReactElement => {
    ...
    const updateFormat = (f: ExportFormat) => { setFormat(f); saveExportSettings({ format: f, sampleRate, bitDepth }); };
    const updateSampleRate = (sr: number) => { setSampleRate(sr); saveExportSettings({ format, sampleRate: sr, bitDepth }); };
    const updateBitDepth = (bd: number) => { setBitDepth(bd); saveExportSettings({ format, sampleRate, bitDepth: bd }); };
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            logger.error(new Error("Export failed", { cause: err }));
        } finally {
            setExporting(false);
        }
    };
            ...
            <DialogContent className="w-[480px] bg-surface-raised" showCloseButton={!exporting}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <section>
                            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2 block">Sample Rate</label>
                        ...
                        <section>
                            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2 block">Bit Depth</label>
                            <div className="flex flex-wrap gap-1">
                                {bitDepths.map((bd) => (
                                    <Button
                                        key={bd}
                    ...
                    <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="sm" onClick={onClose} disabled={exporting}>Cancel</Button>

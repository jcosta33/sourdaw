...
export async function createRockBandTemplate(): Promise<void> {
    ...
    const drumBus = createBus({
        name: 'Drum Bus',
        devices: [
            { type: 'builtin-compressor', name: 'NY Comp', params: { 'comp-threshold': -28, 'comp-ratio': 8, 'comp-attack': 2, 'comp-release': 80, 'comp-knee': 0, 'comp-makeup': 6 } },
            { type: 'builtin-eq', name: 'Drum Bus EQ', params: { 'eq-low-gain': 2, 'eq-low-freq': 90, 'eq-low-q': 0.9, 'eq-mid-gain': -1.5, 'eq-mid-freq': 450, 'eq-mid-q': 1.2, 'eq-high-gain': 2, 'eq-high-freq': 9000, 'eq-high-q': 0.7 } },
        ],
    });
    const guitarBus = createBus({
        name: 'Guitar Bus',
        devices: [
            { type: 'builtin-convolution-reverb', name: 'Room IR', params: { 'conv-ir': 2, 'conv-mix': 0.35, 'conv-predelay': 12, 'conv-lowcut': 120, 'conv-highcut': 9000 } },
        ],
    });
    ...
    const reverbHall = createBus({
        name: 'Reverb Hall',
        devices: [{ type: 'builtin-reverb', name: 'Hall', params: { 'rev-size': 0.85, 'rev-decay': 3.5, 'rev-damping': 0.3, 'rev-mix': 1 } }],
    });
    const tapeDelay = createBus({
        name: 'Tape Delay',
        devices: [{ type: 'faust-tape-delay', name: 'Tape Delay', params: { delay: 0.375, feedback: 0.4, dry_wet: 1 } }],
    });

    const drumFolder = createFolder({ name: 'Drums' });
    const rockKick = createAudioTrack({ name: 'Kick', parentId: drumFolder.id, devices: [
        { type: 'builtin-eq', name: 'Kick EQ', params: { 'eq-low-gain': 3, 'eq-low-freq': 70, 'eq-low-q': 0.9, 'eq-mid-gain': -3, 'eq-mid-freq': 400, 'eq-mid-q': 1.2, 'eq-high-gain': 2, 'eq-high-freq': 4500, 'eq-high-q': 0.8 } },
        { type: 'builtin-compressor', name: 'Kick Comp', params: { 'comp-threshold': -14, 'comp-ratio': 4, 'comp-attack': 10, 'comp-release': 120, 'comp-knee': 6, 'comp-makeup': 3 } },
    ] });
    const rockSnare = createAudioTrack({ name: 'Snare', parentId: drumFolder.id, devices: [
        { type: 'builtin-eq', name: 'Snare EQ', params: { 'eq-low-gain': 1, 'eq-low-freq': 180, 'eq-low-q': 0.9, 'eq-mid-gain': -1, 'eq-mid-freq': 600, 'eq-mid-q': 1, 'eq-high-gain': 3, 'eq-high-freq': 7000, 'eq-high-q': 0.7 } },
        { type: 'builtin-compressor', name: 'Snare Comp', params: { 'comp-threshold': -16, 'comp-ratio': 4, 'comp-attack': 5, 'comp-release': 100, 'comp-knee': 4, 'comp-makeup': 3 } },
    ] });
    ...
    const rockBass = createInstrumentTrack({
        ...
        extraDevices: [
            { type: 'builtin-eq', name: 'Bass EQ', params: { 'eq-low-gain': 2, 'eq-low-freq': 90, 'eq-low-q': 0.9, 'eq-mid-gain': 1, 'eq-mid-freq': 800, 'eq-mid-q': 1.1, 'eq-high-gain': 0, 'eq-high-freq': 5000, 'eq-high-q': 0.7 } },
            { type: 'builtin-compressor', name: 'Bass Comp', params: { 'comp-threshold': -14, 'comp-ratio': 4, 'comp-attack': 8, 'comp-release': 120, 'comp-knee': 6, 'comp-makeup': 2 } },
        ],
    });

    const guitarFolder = createFolder({ name: 'Guitars' });
    const rhythmL = createAudioTrack({ name: 'Rhythm Gtr L', parentId: guitarFolder.id, pan: -0.8, devices: [
        { type: 'grinder', name: 'Amp L', params: {} },
    ] });
    const rhythmR = createAudioTrack({ name: 'Rhythm Gtr R', parentId: guitarFolder.id, pan: 0.8, devices: [
        { type: 'grinder', name: 'Amp R', params: {} },
    ] });
    const leadGtr = createAudioTrack({ name: 'Lead Gtr', parentId: guitarFolder.id, devices: [
        { type: 'grinder', name: 'Amp Lead', params: {} },
        { type: 'builtin-delay', name: 'Lead Delay', params: { 'delay-time': 375, 'delay-feedback': 0.35, 'delay-mix': 0.22 } },
    ] });
    ...
    const leadVocal = createAudioTrack({ name: 'Lead Vocal', parentId: vocalFolder.id, devices: [
        ...
        { type: 'builtin-eq', name: 'Vox EQ', params: { 'eq-low-gain': -2, 'eq-low-freq': 100, 'eq-low-q': 0.8, 'eq-mid-gain': 1.5, 'eq-mid-freq': 3000, 'eq-mid-q': 1, 'eq-high-gain': 2, 'eq-high-freq': 10000, 'eq-high-q': 0.7 } },
    ] });
    ...
    addDeviceChain(rockKick, [
        { type: 'builtin-compressor', name: 'Kick Punch', params: { 'comp-threshold': -12, 'comp-ratio': 4, 'comp-attack': 15, 'comp-release': 120, 'comp-knee': 4, 'comp-makeup': 2 } },
    ]);
    ...
    const tracks = [
        masterTrack,
        drumBus, guitarBus, reverbPlate, reverbHall, tapeDelay,
        drumFolder, rockKick, rockSnare, rockHat, ohL, ohR, rockTom,
        rockBass,
        guitarFolder, rhythmL, rhythmR, leadGtr,
        vocalFolder, leadVocal, backupL, backupR,
    ];

...
export const newProject = (name = "Untitled Project"): void => {
...
export const exportProjectFile = (): void => {
  const tracks = trackStore.value;
  const transport = transportStore.value;
  const automation = automationStore.value;
  const midi = midiStore.value;
  ...
  a.href = url;
  a.download = `${project.name.replace(/[^a-zA-Z0-9_-]/g, "_")}.webdaw`;
  a.click();
  URL.revokeObjectURL(url);
  notifyUser("Project exported successfully", "info");
};

export const importProjectFile = async (file: File): Promise<boolean> => {
  try {

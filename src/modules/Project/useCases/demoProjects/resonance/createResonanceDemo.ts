...
export async function demo1_TheCompleteMix(): Promise<void> {
    ...
    for (let b = 196; b < 224; b += 2) {
        noiseN.push(note(60, b, 1.8, hv(30 + (b - 196) * 2, 5)));
    }
    for (let b = 356; b < 384; b += 2) {
        noiseN.push(note(60, b, 1.8, hv(30 + (b - 356) * 2, 5)));
    }
    ...
    midiStore.set({
        notesByClipId: {
            ...
            [shimmerClip.id]: shimmerN.map((node) => ({ ...node, startBeat: node.startBeat - 128 })),
            [darkClip.id]: darkN.map((node) => ({ ...node, startBeat: node.startBeat - 192 })),
            [strSoftClip.id]: strSoftN.map((node) => ({ ...node, startBeat: node.startBeat - 64 })),
            [strBrightClip.id]: strBrightN.map((node) => ({ ...node, startBeat: node.startBeat - 224 })),
            [leadClip.id]: leadN.map((node) => ({ ...node, startBeat: node.startBeat - 160 })),
            [leadSoftClip.id]: leadSoftN.map((node) => ({ ...node, startBeat: node.startBeat - 224 })),
            [brassClip.id]: brassN.map((node) => ({ ...node, startBeat: node.startBeat - 224 })),
            [arpClip.id]: arpN.map((node) => ({ ...node, startBeat: node.startBeat - 64 })),

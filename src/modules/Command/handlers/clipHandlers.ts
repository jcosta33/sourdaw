...
import { addClip, removeClip, moveClip, duplicateClip, duplicateClipToNextBar } from "#/modules/Track/useCases/clipUseCases";
import { splitClip, trimClipStart, trimClipEnd, setClipFade, normalizeClip, reverseClip, glueClips, nudgeClip, setClipGain, setClipColor, lockClip, crossfadeClips, renameClip, muteClip } from "#/modules/Track/useCases/clipEditingUseCases";
...
export const clipHandlers = {
    ...
    setClipLoopLength: {
        execute: (a) => { setClipLoopLength(a.payload.clipId, a.payload.loopLength); },
    ...
    bounceSelection: {
        execute: (a) => { bounceSelection(a.payload.trackId, a.payload.startBeat, a.payload.endBeat); },
        describe: () => ({ label: "Bounce selection to audio" }),
        undoable: true,
    } satisfies ActionHandler<Extract<AppAction, "bounceSelection">>,

    muteClip: {
        execute: (a) => { muteClip(a.payload.clipId, a.payload.muted); },
    ...
    deleteTime: {
        execute: (a) => { deleteTime(a.payload.startBeat, a.payload.endBeat); },
        describe: () => ({ label: "Delete time" }),
        undoable: true,
    } satisfies ActionHandler<Extract<AppAction, "deleteTime">>,

    insertTime: {
        execute: (a) => { insertTime(a.payload.atBeat, a.payload.durationBeats); },
        describe: () => ({ label: "Insert time" }),
        undoable: true,
    } satisfies ActionHandler<Extract<AppAction, "insertTime">>,

    duplicateTimeRange: {
        execute: (a) => { duplicateTimeRange(a.payload.startBeat, a.payload.endBeat); },
        describe: () => ({ label: "Duplicate time range" }),
        undoable: true,
    } satisfies ActionHandler<Extract<AppAction, "duplicateTimeRange">>,

    stripSilence: {
        execute: (a) => { stripSilence(a.payload.clipId, a.payload.threshold, a.payload.minDuration); },

...
import { toggleMixer, toggleSidebar, toggleInspector, setSnapValue, zoomToFit, zoomToSelection } from "#/modules/Workspace/useCases/togglePanel";
import { setEditingTool } from "#/modules/Workspace/useCases/setEditingTool";
import { addMarker, removeMarker, setMarkerColor, addSection, removeSection, renameSection } from "#/modules/Timeline/useCases/markerUseCases";
import { quantizeNotes, quantizeNoteLengths, transposeNotes, humanizeNotes, invertNotes, retrogradeNotes, scaleVelocities, scaleAllVelocities, setAllVelocities } from "#/modules/Track/useCases/midiUseCases";
import type { VelocityCurve } from "#/modules/Track/useCases/midiUseCases";
import { addAutomationLane, addAutomationPoint, removeAutomationPoint } from "#/modules/Track/useCases/automationUseCases";
...
export const workspaceHandlers = {
    setWorkspaceMode: {
        execute: (a) => { setWorkspaceMode(a.payload.mode); },
    ...
    importMidiFile: {
        execute: () => {
            void pickFiles({ filters: [{ name: "MIDI", extensions: ["mid", "midi"] }] }).then((files) => {
                if (files) {
    ...
    setSnapValue: {
        execute: (a) => { setSnapValue(a.payload.value); },
    ...
    importAudioFile: {
        execute: () => {
            void pickFiles({ filters: [{ name: "Audio", extensions: ["wav", "mp3", "ogg", "flac", "aiff", "aac"] }] }).then((files) => {
                if (files) {
                    for (const file of files) {
    ...
    exportMidi: {
        execute: (a) => { exportMidiClip(a.payload.clipId); },

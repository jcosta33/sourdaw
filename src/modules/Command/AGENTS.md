...
- `useCases`:
    ...
    - **Grounding & Policies**: `getAppActionExecutionPolicy`, `getAppActionStaticAuthority`, `getAgentActionRiskPolicy`, `getAppActionPreviewExecution`, `getExecutableAppActionToolSchemas`, `getExecutableAppActionGroundingCatalog`, `getExecutableAppActionGroundingRules`, `getExecutableCommandRegistrations`, `isExecutableAppActionType`, `expandMidiTransform`, `getMidiTransformContract`, `getMidiTransformToolSchemas`, `selectExecutableAppActionToolSchemasForPrompt`, `requiresAppActionConfirmation`, `configureCommandBatchIdempotency`, `buildSemanticProjectDiff`, `compilePartialCommandBatchAcceptance`, `classifyCommandRecovery`, `describeCommandBatchRecovery`, `commandBatchGroupId`, `getCommandBatchGroupDependencies`, `previewVersionedCommandBatchEnvelope` — whose `previewed` result owns an isolated workspace the caller must release.
...
- A dispatch a person triggered goes through `executeUserAppAction`, never a bare `void executeAppAction(...)`: the admission gates reject rather than return, and a dropped promise turns a refusal the user must see into an unhandled rejection. It notifies on `AppActionConflictError` only and rethrows everything else, so it never launders a real failure.
...
- A static parameter edit carrying `automationRecordingPolicy: 'suppressed'` — available on `setDeviceParameter`, `setTrackGain` and `setTrackPan` — never starts, extends, or flushes a gesture automation recording pass, and neither does its inverse, its redo, or any persisted replay of it. The policy is content, not caller context: it travels in the command's own arguments, so it is admitted by the generated runtime schema only and deliberately absent from the provider-facing descriptor `parameters`, it is covered by the arguments digest and the batch content hash, and it survives binding resolution and partial acceptance unchanged. The owning handler copies it into the inverse and redo it describes and refuses a persisted entry whose forward, inverse and redo disagree about it, because a half-suppressed entry would replay a model's static edit as a fader ride. Absent means the ordinary manual gesture behaviour.
- `getAgentActionRiskPolicy`'s `unsupportedPreviewDomains` signal names the domains whose owner could not preview the batch. It is neither a rejection nor a risk class: the operation stays as risky as it was and its required trust mode is unchanged, but the decision becomes `confirm`, so a change nobody could inspect is accepted by a person rather than direct-committed. `getAppActionPreviewExecution` reports the registered handler's preview capability and answers `unknown` for an unregistered type, which is not previewable either.

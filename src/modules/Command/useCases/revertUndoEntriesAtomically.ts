...
const replayOptions = { skipUndo: true, skipMacroRecording: true } as const;

async function applyInverse(owner: CommandMutationOwner, entry: UndoEntry): Promise<void> {
    if (entry.kind === 'callback') {
        await runCommandHistoryReplay(owner, entry.undo);
        return;
    }
    if (entry.inverseAction) {
        await executeAppActionImpl(entry.inverseAction, replayOptions, owner);
    }
}

function createAdjustmentAggregateInverse(entries: readonly UndoEntry[]): AppAction | null {
...
export async function revertUndoEntriesAtomically(
    ...
    if (entries.length === 1) {
        ...
        await applyInverse(owner, entry);
        return true;
    }

    const aggregateInverse = createAdjustmentAggregateInverse(entries);
    if (!aggregateInverse) {
        return false;
    }
    await executeAppActionImpl(aggregateInverse, replayOptions, owner);

...
export async function getVersionedCommandBatchIdempotentReplay(input: {
    ...
    // Absence is expressed only by the explicit statuses below. Checkpoint and
    // repository read failures must surface: swallowing them as null would
    // misreport unreadable commit evidence as "no prior receipt".

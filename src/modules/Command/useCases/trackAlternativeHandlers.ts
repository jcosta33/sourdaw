...
export const handleCreateTrackAlternative = inject({ getTrackStoreState, setTrackStoreState })(
    ({ getTrackStoreState, setTrackStoreState }) =>
        function handleCreateTrackAlternative(
            action: Extract<AppAction, { type: 'createTrackAlternative' }>
        ): void {
...
export const handleSwitchTrackAlternative = inject({ getTrackStoreState, setTrackStoreState })(
    ({ getTrackStoreState, setTrackStoreState }) =>
        function handleSwitchTrackAlternative(
            action: Extract<AppAction, { type: 'switchTrackAlternative' }>
        ): void {
            const state = getTrackStoreState();
            if (!state) {
                return;
...
export const handleRenameTrackAlternative = inject({ getTrackStoreState, setTrackStoreState })(
    ({ getTrackStoreState, setTrackStoreState }) =>
        function handleRenameTrackAlternative(
            action: Extract<AppAction, { type: 'renameTrackAlternative' }>
        ): void {
            const state = getTrackStoreState();
            if (!state) {
                return;
...
export const handleDeleteTrackAlternative = inject({ getTrackStoreState, setTrackStoreState })(
    ({ getTrackStoreState, setTrackStoreState }) =>
        function handleDeleteTrackAlternative(
            action: Extract<AppAction, { type: 'deleteTrackAlternative' }>
        ): void {
            const state = getTrackStoreState();
            if (!state) {
                return;

 ...
 * Command cannot import Arrangement's concrete models (model isolation — AGENTS.md), so
 * these types specify only the minimum structural guarantees needed by the emitter and
 * the restore handler. Extra fields are preserved via `Record<string, unknown>` so the
 * full shape round-trips without loss.
 */
 ...
 *  action. Mirrors `commandQueries.ts`; specifies only the fields a transform-undo
 *  round-trips (Command cannot import Automation's `AutomationPoint` model). */
...
export type AppAction =
    ...
    | {
           ...
           *  `removeTrack` handler's `describe()` — not invoked directly. The Arrangement
           *  restore handler widens these structural shapes back to its concrete models. */
          type: 'restoreTrack';
          payload: {
              trackId: string;
    ...
    | { type: 'setEditingTool'; payload: { tool: 'select' | 'cut' | 'draw' | 'automation' | 'stretch' | 'marquee' } }
    | {
    ...
    | {
          /** Inverse of `addAutomationLane`, keyed by `(trackId, parameterId)` (the lane
           *  id is not known pre-execute). Mirrors `commandQueries.ts` and
    ...
    | {
           ...
           *  quantize/invert). Restores a lane's `points` to a pre-execute snapshot.
           *  Mirrors `commandQueries.ts` and `AiRuntime/models/RuntimeAction.ts`. */

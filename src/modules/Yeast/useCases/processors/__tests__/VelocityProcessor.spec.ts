...
describe('VelocityProcessor', () => {
    it('passthrough mode preserves velocity', () => {
        ...
        if (out[0]?.kind.type === 'noteOn') expect(out[0].kind.velocity).toBe(80);
    });
    ...
    it('fixed mode sets all notes to fixed velocity', () => {
        ...
        if (out[0]?.kind.type === 'noteOn') expect(out[0].kind.velocity).toBe(64);
    });
    ...
    it('compress mode reduces dynamic range', () => {
        ...
        if (out[0]?.kind.type === 'noteOn') expect(out[0].kind.velocity).toBeLessThan(127);
    });
    ...
    it('curve soft mode shapes velocity', () => {
        ...
        if (out[0]?.kind.type === 'noteOn') expect(out[0].kind.velocity).toBeGreaterThan(0);
    });
    ...
    it('clamps velocity to MIDI range', () => {
        ...
        if (out[0]?.kind.type === 'noteOn') expect(out[0].kind.velocity).toBeLessThanOrEqual(127);
    });
});

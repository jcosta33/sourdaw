...
describe('Arpeggiator', () => {
    ...
    it('publishes the final tied lifetime before a held note releases in a later block', () => {
        ...
        expect(firstBlock.filter(isNoteOn)).toEqual([expect.objectContaining({ durationSamples: 33_600 })]);

        const releaseBlock: MidiEvent[] = [];
        arp.processMidi([{ timeSamples: 12_001, kind: { type: 'noteOff', channel: 0, note: 60 } }], releaseBlock, {
            ...transport,
            blockStartSamples: 12_001,
            blockEndSamples: 24_001,
        });
        expect(releaseBlock).toEqual([]);
        ...
        expect(finalBlock).toEqual([
            expect.objectContaining({ timeSamples: 45_600, kind: { type: 'noteOff', channel: 0, note: 60 } }),
        ]);
    });

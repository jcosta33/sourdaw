/**
 * Orchestral parameter bridge — throttled UI → audio communication.
 *
...
import { type OrchestraPatch } from '../models/OrchestraPatch';
import { orchestralStore, setOrchestraParam, setMacro } from '../stores/orchestralStore';

// ---------------------------------------------------------------------------
// Device reference cache
// ---------------------------------------------------------------------------

type OrchestraDevice = {
    setParam: (name: string, value: number) => void;
    handleCc: (cc: number, value: number) => void;
};

let activeDevice: OrchestraDevice | null = null;

export function registerOrchestraDevice(device: OrchestraDevice): void {
    activeDevice = device;
}

export function unregisterOrchestraDevice(): void {
    activeDevice = null;
...
function getDevice(): OrchestraDevice | null {
    return activeDevice;
}
...
/**
 * Set an orchestral parameter on both UI store and audio engine.
 * Audio updates are throttled to one per rAF per parameter.
 */
export function setOrchestraParamWithAudio<K extends keyof OrchestraPatch>(
    key: K,
    value: OrchestraPatch[K],
): void {
    // Update UI store immediately.
    setOrchestraParam(key, value);

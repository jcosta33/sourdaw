...
const mocks = vi.hoisted(() => ({
    getPluginState: vi.fn<(instanceId: string) => Promise<number[]>>(),
    setPluginState: vi.fn<(instanceId: string, state: number[]) => Promise<void>>(),
}));
...
describe('native plugin state round-trip', () => {
    ...
    it('reads a byte chunk as base64', async () => {
        mocks.getPluginState.mockResolvedValue([1, 2, 3, 255]);
    ...
    it('returns empty string for an empty chunk (absent plugin / dev mode)', async () => {
        mocks.getPluginState.mockResolvedValue([]);

 ...
 * than an effect.
 *
 * `registerFaustDSP` records this as a real flag and tags the WAM descriptor
 * category from it, so consumers must ask rather than infer it from the
 * `faust-` id prefix — every Faust module, effect or instrument, carries that
 * prefix.
 */

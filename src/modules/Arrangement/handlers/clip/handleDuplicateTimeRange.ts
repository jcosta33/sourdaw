...
type PendingDuplicateTimeRangeDescription = {
    label: string;
...
// Keyed by action so concurrent duplications cannot cross. `execute()` mutates the very
// object `describe()` already returned to the caller, which is how `executeAppAction` —
// reading that object only after execution completes — ends up with the real plan.
const pendingDescriptions = new WeakMap<object, PendingDuplicateTimeRangeDescription>();
...
export const handleDuplicateTimeRange = createHandler<'duplicateTimeRange'>({
    execute: (action) => {
        ...
        if (pending) {
            ...
            pending.redoAction = {
                ...
                // the forward operation itself: replaying `duplicateTimeRange` would
                // duplicate the range a second time instead of reinstating the one copy.
    ...
    describe: (action) => {
        const pending: PendingDuplicateTimeRangeDescription = {

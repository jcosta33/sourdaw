...
type PendingDeleteTimeDescription = {
    label: string;
...
// Keyed by action so concurrent deletes cannot cross. `execute()` mutates the very object
...
const pendingDescriptions = new WeakMap<object, PendingDeleteTimeDescription>();
...
export const handleDeleteTime = createHandler<'deleteTime'>({
    execute: (action) => {
        ...
        if (pending) {
            ...
            pending.redoAction = {
                ...
                // the forward operation itself: replaying `deleteTime` would delete a
                // second range instead of reinstating the one this run removed.
    ...
    describe: (action) => {
        const pending: PendingDeleteTimeDescription = {
            label: 'Delete time',

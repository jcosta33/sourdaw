...
function isNativeMidiMessageEvent(event: unknown): event is NativeMidiMessageEvent {
...
function readNativeTimestampMicros(event: NativeMidiMessageEvent): number | undefined {
...
type SelectMidiInputNativeInput = {
    portIndex: number;
    /**
...
export async function selectMidiInputNative({
    portIndex,
    portName,
    onMidiMessage,
    ...
    const openedName = await desktopInvoke('open_midi_input', { portIndex });
    ...
    if (portName !== undefined && typeof openedName === 'string' && openedName !== portName) {
        await desktopInvoke('close_midi_input');
    ...
    setNativeEventUnlisten(newUnlisten);
}

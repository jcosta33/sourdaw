...
export function createDurableAssetOwnershipLifecycle(ownerId: string) {
    return {
        ...
        async rebindOwner(nextOwnerId: string) {
            ...
            const assetStore = transaction.objectStore(ASSET_STORE);
            const leaseStore = transaction.objectStore(LEASE_STORE);
            const [ownedAssetValues, leasedAssetValues, leaseValues] = await Promise.all([
                records.readIndexedValues(assetStore, ASSET_OWNER_INDEX, ownerId),
                records.readIndexedValues(assetStore, ASSET_LEASE_OWNER_INDEX, ownerId),
                records.readIndexedValues(leaseStore, LEASE_OWNER_INDEX, ownerId),
            ]);
            ...
            const assets = [
                ...new Map((indexedAssetValues as AssetRecord[]).map((value) => [value.hash, value])).values(),
            ];
            ...
            for (const asset of assets) {
                ...
                assetStore.put({
                    ...
                    activeLeases: ownsLease
                        ...
                        : asset.activeLeases,
                    leaseOwnerIds: ownsLease
                        ? [...new Set(asset.leaseOwnerIds.map((id) => (id === ownerId ? nextOwnerId : id)))]
                        : asset.leaseOwnerIds,
            ...
            for (const hash of reboundHashes) {
                records.notifyInvalidation({ hash, ownerId });
            }
            return { status: 'rebound' as const, previousOwnerId: ownerId, ownerId: nextOwnerId, reboundHashes };
        },
    };
}

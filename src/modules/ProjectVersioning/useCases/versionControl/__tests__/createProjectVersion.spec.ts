...
const mocks = vi.hoisted(() => ({
    ...
    captureSnapshot: vi.fn<() => ProjectSnapshot>(),
}));
...
describe('createProjectVersion', () => {
    beforeEach(() => {
        ...
        mocks.captureSnapshot.mockReturnValue({ data: '{}', size: 2 });
    });
    ...
    it('does nothing without store state; else appends a version, moves the branch head, and defaults tags', () => {
        ...
        createProjectVersion('Label', 'desc', ['t1']);
        ...
        expect(version.snapshot).toEqual({ data: '{}', size: 2 });

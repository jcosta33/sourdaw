...
const TRACK_COLORS = [
    "oklch(0.65 0.15 145)", "oklch(0.65 0.15 260)", "oklch(0.65 0.15 50)",
    "oklch(0.65 0.15 0)", "oklch(0.65 0.15 310)", "oklch(0.65 0.15 180)",
    "oklch(0.65 0.15 90)", "oklch(0.65 0.15 30)", "oklch(0.65 0.15 220)",
    "oklch(0.65 0.08 200)",
];
...
export const TrackContextMenu = ({ track, children }: TrackContextMenuProps): ReactElement => {
    ...
    const close = () => { setPosition(null); setRenaming(false); };
    ...
    const handleDuplicate = () => {
        ...
        if (newTrack) {
            for (const clip of track.clips) {
                addClip({ trackId: newTrack.id, startBeat: clip.startBeat, endBeat: clip.endBeat, name: clip.name, type: clip.type, audioBufferId: clip.audioBufferId });
            }
        }
        close();
    };
    ...
    const actions: (MenuItem | { label: "---" })[] = [
        { label: "Add Clip", action: () => { addClip({ trackId: track.id, startBeat: 0, endBeat: 16, name: `Clip ${Date.now() % 1000}`, type: track.kind === "midi" ? "midi" : "audio" }); close(); } },
        ...
        { label: "---" },
        { label: track.armed ? "Disarm" : "Arm for Recording", action: () => { armTrack(track.id, !track.armed); close(); } },
        { label: track.soloSafe ? "Disable Solo Safe" : "Solo Safe", action: () => { toggleSoloSafe(track.id); close(); } },
        ...((track.kind === "audio" || track.kind === "midi") ? [
            { label: `Input Monitor: ${track.inputMonitoring.charAt(0).toUpperCase() + track.inputMonitoring.slice(1)}`, action: () => setShowInputMon(true) },
        ] : []),
        { label: track.frozen ? "Unfreeze" : "Freeze", action: () => { track.frozen ? unfreezeTrack(track.id) : freezeTrack(track.id); close(); } },
        { label: "Bounce in Place", action: () => { bounceInPlace(track.id); close(); } },
        { label: "Bounce to New Track", action: () => { bounceToNewTrack(track.id); close(); } },
        { label: "---" },
        ...
        <div onContextMenu={handleContextMenu}>
            <input
                ...
                onChange={(e) => {
                    ...
                    if (file) void handleImportAudio(file);
                    e.target.value = "";
                }}
            />
            <input
                        ...
                        {renaming ? (
                            <div className="px-2 py-1">
                                <input
                                    ...
                                    onKeyDown={(e) => { if (e.key === "Enter") handleRenameCommit(); if (e.key === "Escape") close(); }}
                                ...
                                <div className="grid grid-cols-5 gap-1">
                                    {TRACK_COLORS.map((color) => (
                                        <button
                                            ...
                                            onClick={() => { setTrackColor(track.id, color); close(); }}
                            ...
                            <div className="p-1">
                                ...
                                {INPUT_MON_OPTIONS.map((opt) => (
                                    <button
                                        ...
                                        onClick={() => { setInputMonitoring(track.id, opt.value); close(); }}
                                    >
                                        {opt.label}
                                    </button>
                                ))}

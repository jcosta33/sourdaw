...
import { setNotesForClip } from './midiNoteCrud';

export function createAlternativeClips(originalClipId: string, variationsData: Array<Array<{pitch: number; startBeat: number; duration: number; velocity: number}>>): void {
    const state = getTrackState();
    if (!state) return;
    ...
    if (!targetTrack || !originalClip) return;

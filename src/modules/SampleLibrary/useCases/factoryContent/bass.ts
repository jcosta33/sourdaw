...
function renderSineSub(): Float32Array {
    ...
    const env = renderEnvelope(dur, { attack: 0.01, decay: 0.05, sustainLevel: 0.85, sustain: 1, release: 0.4, curve: 'exp' });
...
function renderSquareReese(): Float32Array {
    ...
    const env = renderEnvelope(dur, { attack: 0.02, decay: 0.1, sustainLevel: 0.7, sustain: 0.8, release: 0.4, curve: 'exp' });
...
function renderWobbleBass(): Float32Array {
    ...
    const env = renderEnvelope(dur, { attack: 0.02, decay: 0.1, sustainLevel: 0.7, sustain: 0.9, release: 0.4, curve: 'exp' });
    applyEnvelope(out, env);
    softClip(out, 1.3);
    normalize(out, 0.9);
    return out;
...
function renderSawBass(): Float32Array {
    ...
    const env = renderEnvelope(dur, { attack: 0.005, decay: 0.1, sustainLevel: 0.7, sustain: 0.9, release: 0.3, curve: 'exp' });
    applyEnvelope(out, env);
    softClip(out, 1.15);
    normalize(out, 0.9);
    return out;
...
function renderTb303(): Float32Array {
    ...
    const env = renderEnvelope(dur, { attack: 0.003, decay: 0.08, sustainLevel: 0.35, sustain: 0, release: 0.65, curve: 'exp' });
    applyEnvelope(out, env);
    softClip(out, 1.4);
    normalize(out, 0.9);
    return out;
...
function renderPadBass(): Float32Array {
    ...
    const env = renderEnvelope(dur, { attack: 0.15, decay: 0.2, sustainLevel: 0.75, sustain: 1.1, release: 0.5, curve: 'exp' });
    applyEnvelope(out, env);
    normalize(out, 0.85);
    return out;
}

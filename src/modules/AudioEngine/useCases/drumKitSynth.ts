import { type SynthParams, defaultSynthParams, scheduleNote } from "#/modules/AudioEngine/useCases/builtinSynth";
...
const FACTORY_KITS: readonly DrumKit[] = [KIT_808, KIT_ANALOG, KIT_ELECTRONIC, KIT_ACOUSTIC];

const kitIndex = new Map<string, DrumKit>(FACTORY_KITS.map((k) => [k.id, k]));

export const getFactoryDrumKits = (): readonly DrumKit[] => FACTORY_KITS;

export const getDrumKitById = (id: string): DrumKit | null => kitIndex.get(id) ?? null;

const KIT_ID_BY_INDEX: Record<number, string> = Object.fromEntries(
    FACTORY_KITS.map((k, i) => [i, k.id]),
);

export const getDrumKitByIndex = (index: number): DrumKit | null =>
    getDrumKitById(KIT_ID_BY_INDEX[index] ?? "");

const findVoice = (kit: DrumKit, pitch: number): DrumKitVoice | null => {

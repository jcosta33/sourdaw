 ...
 *   - **Bus → track routing** — a bus routed at an ordinary track is still
 *     gated here. A bus whose output names the master track resolves to the
 *     master target (`resolveOutputTarget`), so the default bus output is not
 *     this gate.
 *

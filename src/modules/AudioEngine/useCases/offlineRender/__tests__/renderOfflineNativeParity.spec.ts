...
function inProcessNativeTransport(host: NativeHostAddon): NativeGraphTransport {
    return {
        ...
        async beginLevainBank() {
            throw new Error('the export parity spec stages no levain bank (begin_levain_bank)');
        },
        async registerLevainSample() {
            throw new Error('the export parity spec stages no levain bank (register_levain_sample)');
        },

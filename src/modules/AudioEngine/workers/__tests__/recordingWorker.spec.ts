...
describe('acquireRingChunk', () => {
    ...
    it('keeps the actual producer and consumer coherent across the signed Int32 count boundary', async () => {
        ...
        const afterFirst = writeRingRelease(ring, control, boundaryHead, Float32Array.from([0.125, -0.25, 0.5, -1]));
        ...
        const afterSecond = writeRingRelease(ring, control, afterFirst, Float32Array.from([0.75, -0.875]));
    ...
    it('keeps a real 128-frame worklet block that straddles the signed boundary intact', async () => {
        ...
        const after = writeRingRelease(ring, control, head, block);

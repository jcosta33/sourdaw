...
import { GLUTEN_DSP_PARAM_NAMES, mapGlutenParamToDspParam } from '../GlutenDspParamNames';

/**
 * Gluten's two hosts spell one compressor's parameters, and this is the table
 * they share.
 *
 ...
 */
describe('Gluten DSP parameter names', () => {
    it('names every parameter the descriptor declares', () => {
        const descriptor = getPluginById('gluten');
        if (descriptor === undefined) {
            throw new Error("no builtin descriptor for 'gluten'");
        }

        expect(descriptor.parameters.length).toBeGreaterThan(0);
        for (const parameter of descriptor.parameters) {
            expect(mapGlutenParamToDspParam({ paramId: parameter.id })).not.toBeNull();
        }
    });
    ...
    it('resolves a project id, and refuses the engine spelling or an unknown id', () => {
        expect(mapGlutenParamToDspParam({ paramId: 'autoMakeup' })).toBe('auto_makeup');
        expect(mapGlutenParamToDspParam({ paramId: 'auto_makeup' })).toBeNull();
        expect(mapGlutenParamToDspParam({ paramId: 'bogus' })).toBeNull();
    });
});

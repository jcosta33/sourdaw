...
export const PARAMETER_LEVEL_OFFLINE_AUTOMATION_EXEMPTIONS: Readonly<Record<string, Readonly<Record<string, string>>>> =
    {
        fermenter: {
            // Both survivors are absences of engine behaviour, not absences of
            // wiring: no two values of either of them render differently, so an
            ...
            //
            // `portamentoMode` left this table when the engine started reading
            // the field: `Layer::portamento_time_for_note_on` now suppresses the
            // glide in legato mode when no key is down, `crates/daw-dsp/tests/
            // fermenter_portamento_mode_legato.rs` pins that behaviour, and the
            // ordinal probe measures the render delta the binding needs.
            activeLayer:

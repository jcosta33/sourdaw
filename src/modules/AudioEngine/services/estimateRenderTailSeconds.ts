...
function evaluateDeclaration(device: DeviceLike, tail: TailDeclarationLike): number {
    ...
    if (tail.kind === 'mappedDecaySeconds') {
        // The stored value is normalised; the law owns what it means in seconds.
        const normalised = readParameter(device, tail.parameterId, tail.defaultValue);
        return withPredelay(device, decayToRt60Seconds(normalised), tail.predelayMsParameterId);
    }
...
export type EstimateRenderTailSecondsOutput = {
    ...
    seconds: number;
    /**
     * The same total before the ceiling was applied. A caller that feeds this
     * estimate back into another estimate must carry this rather than `seconds`,
     * or a pre-clamped value re-enters as if it were the real total and the
     * outer clamp check silently passes.
     */

...
/** Retune speeds handed to the WASM instance, one entry per engine-fed quantum. */
const retuneSpeedCalls: number[] = [];
/** Formant-preservation flags handed to the WASM instance, same cadence. */
const formantPreserveCalls: boolean[] = [];
...
class KneadInstanceMock {
    ...
    set_retune_speed_ms(value: number): void {
        retuneSpeedCalls.push(value);
    }
    set_formant_preserve(value: boolean): void {
        formantPreserveCalls.push(value);
    }
...
describe('KneadProcessor pitch-shift computation', () => {
    beforeEach(() => {
        shiftCalls.length = 0;
        retuneSpeedCalls.length = 0;
        formantPreserveCalls.length = 0;
    ...
    /**
     * `retuneSpeedMs` and `formantPreserve` ride the same `syncKneadState`
     * payload the blobs do; the processor used to declare neither and read
     * neither, so both sliders were inert no matter what the panel said.
     */
    it('forwards the clip retune speed and formant flag to the WASM instance', async () => {
        const proc = await loadProcessor();
        const transportSAB = makePlayingTransport(2, 120);
        sendMessage(proc, { type: 'init', wasmModule: MINIMAL_WASM_MODULE, transportSAB });
        ...
        expect(retuneSpeedCalls.at(-1)).toBe(140);
        expect(formantPreserveCalls.at(-1)).toBe(false);
    });

    /**
     * A clip record persisted before these fields existed carries neither, and
     * a bare read would hand the engine `undefined` — which crosses the wasm
     * boundary as 0 for the number and `false` for the flag, i.e. a snap and a
     * dropped formant correction on every reloaded project. The fallbacks are
     * the panel's own `?? 25` / `?? true`.
     */
    it('falls back to the panel defaults for a clip persisted without the two fields', async () => {
        const proc = await loadProcessor();
        const transportSAB = makePlayingTransport(2, 120);
        sendMessage(proc, { type: 'init', wasmModule: MINIMAL_WASM_MODULE, transportSAB });
        ...
        expect(retuneSpeedCalls.at(-1)).toBe(25);
        expect(formantPreserveCalls.at(-1)).toBe(true);
    });

    /**
     * A non-finite retune speed in the payload must not reach the engine,
     * where it would be fed to `exp()` and poison every subsequent glide step
     * into NaN — silencing the track rather than mistiming it.
     */
    it('refuses a non-finite retune speed and keeps the default', async () => {
        const proc = await loadProcessor();
        const transportSAB = makePlayingTransport(2, 120);
        sendMessage(proc, { type: 'init', wasmModule: MINIMAL_WASM_MODULE, transportSAB });

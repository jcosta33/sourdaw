...
- **Grand Boule runtime**: Grand Boule runs its live WASM engine in a Worker behind a SharedArrayBuffer ring. Offline render runs the same engine inline in an AudioWorklet, where no live deadline exists. Notes reach the engine with a sample offset inside the block about to render, so a scheduled note sounds on its own frame; params and pedals are block-rate and apply at the head.

...
export function enqueueRender(entry: RenderQueueEntry): void {
    renderQueueStore.update((state) => {
        ...
        return {
            ...state,
            // One phrase has one current owner. Re-launching the same phrase replaces
            // its older queue entry so late completion/failure/cancellation can be
            // rejected by requestId instead of mutating the new render.

...
export const ProofChamber = ({ instanceId }: { instanceId: string }): ReactElement => {
    ...
    if (!state) return <div className="p-4">Loading Dutch Oven...</div>;
        ...
        <div className="flex flex-col w-[600px] h-[500px] bg-card text-card-foreground border rounded-lg shadow-xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b bg-background/50">
                <div className="flex items-center gap-2">
                    <span className="font-bold tracking-widest text-sm uppercase opacity-80">Dutch Oven</span>
                    <span className="text-xs px-2 py-0.5 rounded-full bg-accent/20 text-accent-foreground border border-accent/30 mix-blend-screen">
                        {engineState.algorithm}
                    </span>
                </div>
            ...
            {uiLevel >= 2 && (
                <div className="grid grid-cols-4 gap-6 px-6 py-4 bg-background/30 flex-1">
                    ...
                    <KnobControl
                        ...
                        max={100}
                        onChange={(val) =>
                            updateChamberEngine(instanceId, (s) => ({ ...s, modulationDepth: val / 100 }))
                        }

import { trackStore, getEffectiveGain } from '#/modules/Arrangement';
        ...
        if (lane.parameterId === 'gain') {
            ...
            for (const device of track.devices) {
                if (device.parameterValues[lane.parameterId] !== undefined) {
                    // Apply a single-pole IIR slew to smooth step-function jumps
                    // and skip dispatch when the value has converged (flat curve).

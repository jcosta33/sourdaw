 ...
 */
export function humanizeNotes(clipId: string, amount: number, seed?: number): number {
    ...
    midiStore.set({
        ...
        notesByClipId: {
            ...
            [clipId]: existing.map((n) => ({
                ...n,
                startBeat: n.startBeat + (rng() - 0.5) * amount * 0.25,
                velocity: Math.max(1, Math.min(127, n.velocity + Math.round((rng() - 0.5) * amount * 10))),
            })),
        },
    });

...
- **Use Cases** (`useCases/index.ts`): `initBacteriaSubscribers`, `initBacteriaModAssignmentsPersistence`, `captureOfflineBacteria`, `prepareOfflineBacteria`, `resolveNativeBacteriaModAssignments`. Internal parameter bridge and preset loaders otherwise consume state within the module.
...
- The table reaches the native engine too, at every route the Web Audio one does (#4685 slice 2). At build — live session start and native offline export alike — `projectDeviceForNativeBody` asks the composition root's `nativeModAssignments` sink hook (`resolveNativeBacteriaModAssignments`), which decodes `deviceState` and applies the same shared refusal rules as `prepareOfflineBacteria` (`resolveMappedBacteriaModAssignments` in `models/BacteriaModulationIds.ts` — one helper, so the two carriers cannot drift on which tables they refuse), and folds the rows into the device the native body receives. On every live edit, `updateDevicePatch` sends a natively carried device's `modAssignments` in the same batch as its parameter write, so the native engine never lags the Web Audio worklet by a render.

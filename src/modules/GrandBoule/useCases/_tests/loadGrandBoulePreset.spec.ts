...
describe('loadGrandBoulePreset', () => {
    ...
    it('should return false when preset id is unknown', () => {
        const findBuiltinGrandBoulePreset = vi.fn(() => null);
        injectDependencies(loadGrandBoulePreset, { findBuiltinGrandBoulePreset });

        const engine = { setParam: vi.fn() };
        expect(loadGrandBoulePreset({ engine, presetId: 'nope' })).toBe(false);
    ...
    it('should return false when preset exists but store is empty', () => {
        ...
        const findBuiltinGrandBoulePreset = vi.fn(() => ({
            id: 'ok',
            name: 'OK',
            description: '',
            parameters: params,
        ...
        const engine = { setParam: vi.fn() };
        expect(loadGrandBoulePreset({ engine, presetId: 'ok' })).toBe(false);
    });
    ...
    it('should load preset into store and engine when preset and store exist', () => {
        ...
        expect(loadGrandBoulePreset({ engine, presetId: 'ok' })).toBe(true);

...
import { getTimeSignatureAtBeat } from "#/modules/Transport/models/TimeSignatureMap";
...
const drawBeatRuler = (
  ...
  for (
    let beat = startBeat;
    (beat - viewportStartBeat) * pixelsPerBeat < width;
    beat++
  ) {
...
const drawGrid = (
  ...
  for (const change of [...tsChanges].sort((a, b) => a.beat - b.beat)) {
    ...
    const beatsInSegment = change.beat - barStartBeat;
    barStartBeat = change.beat + (beatsInSegment % currentNumerator === 0 ? 0 : currentNumerator - (beatsInSegment % currentNumerator));
    barStartBeat = change.beat;
    currentNumerator = change.numerator;
  }
  ...
  for (
    let beat = startBeat;
    beat * pixelsPerBeat < width + viewportStartBeat * pixelsPerBeat;
    beat++
  ) {
    const x = (beat - viewportStartBeat) * pixelsPerBeat;
...
const getTrackYOffsets = (tracks: { height: number }[]): number[] => {
...
const drawClip = (
  ...
  if (isMuted) ctx.globalAlpha = 0.35;
  ...
  if (w > 50) ctx.fillText(typeLabel, x + 6, trackY + 23, w - 12);

  if (clip.type === "midi" && clip.midiNotes.length > 0) {
    drawMidiNotePreview(ctx, clip, model, x, trackY, w, trackHeight, padding);
...
const drawTempoMap = (
  ...
  if (!tempoState || tempoState.changes.length === 0) return;
...
const drawTakeLanes = (
  ...
  if (!takeState || takeState.lanes.length === 0) return;

...
import { type Step } from '../models/GrinderKit';
import { triggerGrinderPad } from './triggerPad';
import { setGrinderPadParam } from './grinderParamBridge';
...
function tick(currentStep: number, bpm: number, stepsPerBeat: number): void {
    ...
    for (const track of pattern.tracks) {
        ...
        for (const [key, value] of Object.entries(locks)) {
            ...
            setGrinderPadParam(track.padIndex, key as keyof import('../models/GrinderKit').PadState, value);
        }
        ...
        const fire = () => triggerGrinderPad(track.padIndex, vel);
        ...
        if (step.retriggerCount > 0) {
            ...
            for (let r = 1; r <= step.retriggerCount; r++) {
                ...
                setTimeout(() => triggerGrinderPad(track.padIndex, retrigVel), totalDelayMs + subInterval * r);
            }
        }
    }

    // Update UI cursor
    grinderStore.set({ ...state, currentStep, isPlaying: true });
...
export function stopSequencer(): void {
    ...
    const state = grinderStore.value;
    if (state) {
        grinderStore.set({ ...state, isPlaying: false, currentStep: 0 });
    }
    playCount = 0;
}

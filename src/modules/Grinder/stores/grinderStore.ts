...
import { type GrinderKit, type PadState, createDefaultKit } from '../models/GrinderKit';
...
export function selectPad(index: number): void {
    ...
    if (state && index >= 0 && index < state.kit.pads.length) {
        grinderStore.set({ ...state, selectedPadIndex: index });
    }
}
...
export function updatePad(index: number, updates: Partial<PadState>): void {
    ...
    grinderStore.set({ ...state, kit: { ...state.kit, pads } });
}

export function updateKitParam(key: keyof GrinderKit, value: number | string): void {
    const state = grinderStore.value;
    if (!state) { return; }
    grinderStore.set({ ...state, kit: { ...state.kit, [key]: value } });
}

export function loadKit(kit: GrinderKit): void {
    const state = grinderStore.value;
    if (state) {
        grinderStore.set({ ...state, kit, selectedPadIndex: 0 });
    }
}
...
export function toggleStep(padIndex: number, stepIndex: number): void {
    ...
    grinderStore.set({ ...state, kit: { ...state.kit, patterns: newPatterns } });
}
...
export function setGrinderEngineReady(ready: boolean): void {
    const state = grinderStore.value;
    if (state) {
        grinderStore.set({ ...state, engineReady: ready });
    }
}

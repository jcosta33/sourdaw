...
import {
    grinderStore, selectPad, toggleStep, setStepVelocity, type GrinderState,
} from '../../stores/grinderStore';
import { type PadState } from '../../models/GrinderKit';
import { setGrinderPadParam } from '../../useCases/grinderParamBridge';
import { triggerGrinderPad } from '../../useCases/triggerPad';
import { startSequencer, stopSequencer } from '../../useCases/sequencerPlayback';
import { GRINDER_PRESETS } from '../../repositories/grinderPresets';
import { loadGrinderKitPreset } from '../../useCases/loadGrinderKit';
...
export const GrinderPanel = (): ReactElement => {
    const state = useSyncExternalStore<GrinderState | null>(
        (cb) => grinderStore.subscribe(cb), () => grinderStore.value,
    );
    ...
    const kit = state?.kit ?? grinderStore.value!.kit;
    ...
    const setP = useCallback((key: string, value: number) => {
        setGrinderPadParam(selectedPadIndex, key as keyof PadState, value);
    }, [selectedPadIndex]);

    // Engine type changes are handled by kit presets, not individual pad UI

    const trigPad = useCallback((i: number) => triggerGrinderPad(i, 100), []);
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-3 px-1 py-1 shrink-0 border-b border-border/30 bg-surface-app/30">
                ...
                <div className="relative shrink-0">
                    <select
                        value={GRINDER_PRESETS.find((p) => p.name === kit.name)?.id ?? ''}
                        onChange={(e) => {
                            const preset = GRINDER_PRESETS.find((p) => p.id === e.target.value);
                            if (preset) { loadGrinderKitPreset(preset.kit); }
                        }}
                ...
                <Button variant={isPlaying ? 'secondary' : 'ghost'} size="icon-xs" className="h-5 w-5 shrink-0"
                    onClick={() => { if (isPlaying) { stopSequencer(); } else { startSequencer(120); } }}>

 ...
 * outside, a grant is what admits, revoking it is a different refusal from
 * never having had one, and a grant is bound to the project it was issued
 * against on both sides. The source half is a census over this module's own
 * files, because "an adapter never executes, commits or confirms" is a
 * property of the whole module rather than of any one call: a future file that
 * imported the document, the batch executor or the desktop bridge from the
 * wrong place would pass every behavioural test above and still break it.
 */
...
import { resetExternalClientSession } from '../../stores/externalClientSessionStore';
...
const {
    ...
    getProjectProtocolContractsMock,
    ...
    getProjectProtocolContractsMock: vi.fn(),
...
vi.mock('#/modules/Project/useCases', () => ({
    getProjectProtocolContracts: getProjectProtocolContractsMock,
...
describe('external agent adapter conformance', () => {
    beforeEach(() => {
        ...
        getProjectProtocolContractsMock.mockReturnValue({ query: { operations: [{ name: QUERY_TYPE }] } });
    ...
    it('admits an enabled, granted client and refuses the same client once its grant is revoked', () => {
        ...
        enableExternalClientTransport('mcp');
        expect(
            issueExternalClientGrant({ clientId: CLIENT_ID, transport: 'mcp', operations: ['project.query'] })
        ).not.toBeNull();
    ...
    it('refuses an operation the grant does not name', () => {
        setExternalClientActiveProject(PROJECT_ID);
        issueExternalClientGrant({ clientId: CLIENT_ID, transport: 'cli', operations: ['project.query'] });
    ...
    it('defers an operation whose owner contract is absent from the manifest, and refuses it as unpublished', () => {
        ...
        setExternalClientActiveProject(PROJECT_ID);
        issueExternalClientGrant({ clientId: CLIENT_ID, transport: 'cli', operations: ['receipt.read'] });
    ...
    it('previews a command batch without ever executing one, and hands approval back to the local flow', () => {
        setExternalClientActiveProject(PROJECT_ID);
        issueExternalClientGrant({
            clientId: CLIENT_ID,
            transport: 'cli',
            operations: ['command.preview', 'command.approval'],
        });
    ...
    it('closes a transport without deleting what was granted over it', () => {
        setExternalClientActiveProject(PROJECT_ID);
        enableExternalClientTransport('remote');
        issueExternalClientGrant({ clientId: CLIENT_ID, transport: 'remote', operations: ['project.query'] });

/**
 * Voice command overlay: provides mic button + speech-to-text,
 * injecting transcribed text into the PromptBar.
 *
 * Detection order:
...
export const VoiceCommandOverlay = (): ReactElement | null => {
    ...
    const startBrowserRecognition = (): boolean => {
        ...
        recognition.onerror = (event) => {
            ...
            // 'no-speech' is expected — user just didn't say anything
        };
    ...
    const stopListening = (): void => {
        ...
        if (modeRef.current === 'whisper' && mediaRecorderRef.current) {
            if (mediaRecorderRef.current.state === 'recording') {
                mediaRecorderRef.current.stop();
                return; // onstop callback handles state reset
            }
        }
        setIsListening(false);
    };
    ...
    if (!isListening && !transcribing && !errorText) {
        return null;
    }

    const displayText = errorText || finalText + (interimText ? ` ${interimText}` : '');

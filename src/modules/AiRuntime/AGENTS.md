...
- Configured hard resource limits arm every run: creation copies the configured ceilings into the run's budgets, except the three the lifecycle enforces itself — request size and concurrency refuse creation, and a run past its wall-clock limit reserves no further work. Every refusal is a typed `hard-limit-reached` outcome naming the category, never a throw or a silent pass.

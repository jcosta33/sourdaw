...
- Each owner's local retention policy is declared once in `AgentRetentionPolicy` and enforced by that owner's store. Retention never evicts a non-terminal run, a run holding unreleased temporary assets, or a run a recovery ledger names. Deletion removes owned content only: a run with committed work is purged to its receipts, never removed.

...
- The telemetry projection of an agent run carries no free text the run holds — only identifiers, enum literals, revisions and counts — while the developer-diagnostics projection carries that free text only after secret redaction and only when the caller requests project content.

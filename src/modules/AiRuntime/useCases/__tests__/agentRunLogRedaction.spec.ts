...
describe('redactSecrets', () => {
    ...
    it('replaces a basic authorization value and keeps its label', () => {
        expect(redactSecrets('curl -H "Authorization: Basic YWRtaW46c3VwZXJzZWNyZXQ=" https://gw/v1')).toEqual({
    ...
    it('replaces a quoted key inside a json body', () => {
        expect(redactSecrets('request body was {"apiKey":"a1b2c3d4e5f6a7b8c9d0e1f2"}')).toEqual({
            text: 'request body was {"apiKey":"[redacted]"}',
            redactedCount: 1,
        });
    });
    ...
    it('counts every shape it replaced in one text', () => {
        expect(redactSecrets(`Bearer abcdefghijklmnopqrstuvwxyz012345 then sk-${'b'.repeat(20)}`)).toEqual({

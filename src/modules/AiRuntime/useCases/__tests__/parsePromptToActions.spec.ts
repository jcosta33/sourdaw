    ...
    it('keeps a surviving glue action and rejects its omission from a mixed provider batch', async () => {
        ...
        vi.mocked(generateToolCalls).mockResolvedValue(
            completePlan([
                ...
                { name: 'glueClips', arguments: { clipIds: ['clip-outro', 'clip-coda'] } },
            ])
        );
        const clips = [
            ...
            { id: 'clip-coda', name: 'Coda', startBeat: 24, endBeat: 32 },
        ].map((clip) => ({
            ...clip,
            type: 'midi' as const,
            gain: 1,
        ...
        const result = await parsePromptToActions(
            "glue Intro and Verse clips, then glue Outro and Coda clips, but don't glue Intro and Verse after all",
            providerContext
        );

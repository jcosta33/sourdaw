...
describe('Grid', () => {
    ...
    describe('cols prop', () => {
        it.each([
            ...
            [5, 'grid-cols-5'],
            [6, 'grid-cols-6'],
        ] as const)('applies grid-cols-%i class', (cols, expectedClass) => {
    ...
    describe('gapX prop', () => {
        it.each([
            ...
            [6, 'gap-x-6'],
            [8, 'gap-x-8'],
        ] as const)('applies gap-x-%i class', (gapX, expectedClass) => {
    ...
    describe('gapY prop', () => {
        it.each([
            ...
            [6, 'gap-y-6'],
            [8, 'gap-y-8'],
        ] as const)('applies gap-y-%i class', (gapY, expectedClass) => {
    ...
    describe('flow prop', () => {
        it.each([
            ['row', 'grid-flow-row'],
            ['col', 'grid-flow-col'],
        ] as const)('applies grid-flow-%s class', (flow, expectedClass) => {
            render(<Grid flow={flow} data-testid="grid" />);
            expect(screen.getByTestId('grid')).toHaveClass(expectedClass);
        });

        it('does not apply flow class when undefined', () => {
            render(<Grid data-testid="grid" />);
            const classes = screen.getByTestId('grid').className;
